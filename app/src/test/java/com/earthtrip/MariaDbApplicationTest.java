...
class MariaDbApplicationTest {
    ...
    private static final UUID REQUEST_ID =
        UUID.fromString("63ff50e2-7b97-47cc-ac0e-a4bacb6001a3");
    private static final UUID OWNER_ID =
        UUID.fromString("b5ba2a4e-f274-47f8-8dc0-1392d5774c98");
    ...
    void JPA_version을_갱신한_응답에_즉시_반영한다() {
        ...
        var updated = tripManagementUseCase.update(
            tripId,
            OWNER_ID,
    ...
    void 실패한_내부_웹훅을_작업과_데드레터와_감사이력으로_저장한다() {
        ...
        String body = "{\"fileId\":\"" + UUID.randomUUID()
            + "\",\"result\":\"SAFE\"}";

        var accepted = internalOperationsUseCase.acceptWebhook(
            "malware-scan",
            eventId,
            timestamp,
        ...
        assertThat(internalOperationsUseCase.jobs(
            "FAILED", "MALWARE_SCAN_WEBHOOK", 20
        ...
        assertThat(internalOperationsUseCase.deadLetters("OPEN", 20))
            ...
            .contains(accepted.jobId());
        assertThat(internalOperationsUseCase.auditEvents(
    ...
    void 민감_예약정보는_DB에_AES_GCM_envelope로_저장하고_API에서는_복호화한다() {
        ...
        var created = walletRecordUseCase.create(
            tripId,
            OWNER_ID,
            "RESERVATION",
            false,
        ...
        assertThat(created.payload().get("confirmationNumber"))
            .isEqualTo("SECRET-ABC-1234");
        ...
        assertThat(walletRecordUseCase.get(
            tripId, OWNER_ID, "RESERVATION", reservationId
        ).payload()).isEqualTo(created.payload());
    }
