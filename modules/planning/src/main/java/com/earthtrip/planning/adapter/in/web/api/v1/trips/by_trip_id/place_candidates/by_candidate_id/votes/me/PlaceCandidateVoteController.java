...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/place-candidates/{candidateId}/votes/me")
class PlaceCandidateVoteController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    PlaceCandidateVoteController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    void delete(@PathVariable UUID tripId, @PathVariable UUID candidateId) {
        useCase.deleteUserState(
                tripId, actor.requireUserId(), "PLACE_CANDIDATE", candidateId, "PREFERENCE");
    }
}
