...
@Entity
@Table(name = "planning_resources")
class PlanningResourceJpaEntity {
    @Id
    ...
    PlanningResourceJpaEntity(PlanningResource r, String json) {
        id = r.id().toString();
        apply(r, json);
    }

    void apply(PlanningResource r, String json) {
        tripId = r.tripId().toString();
        resourceType = r.type();
        parentId = r.parentId() == null ? null : r.parentId().toString();
    ...
    PlanningResource toDomain(Map<String, Object> data) {
        return PlanningResource.restore(
                UUID.fromString(id),
