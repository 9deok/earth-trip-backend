...
class RouteOverrideService implements RouteOverrideUseCase {
    ...
    private static final Set<String> MODES =
            Set.of("WALKING", "TRANSIT", "DRIVING", "BICYCLING", "TAXI", "OTHER");
    ...
    RouteOverrideService(
            TripDayUseCase days, ScheduleUseCase schedule, PlanningResourceUseCase resources) {
        this.days = days;
    ...
    public List<OverrideResult> list(UUID tripId, UUID dayId, UUID actorUserId) {
        ...
        return resources
                .list(tripId, actorUserId, "ROUTE_OVERRIDE", day.dayId(), day.localDate())
                .stream()
    ...
    public OverrideResult create(
            UUID tripId, UUID dayId, UUID actorUserId, OverrideCommand command) {
        TripDayUseCase.DayResult day = days.requireDay(tripId, dayId, actorUserId);
        if (command.requestId() == null
                || command.fromItemId() == null
                || command.toItemId() == null
                || command.fromItemId().equals(command.toItemId())) {
            throw EarthTripException.badRequest(
                    "INVALID_ROUTE_OVERRIDE_ITEMS", "서로 다른 출발 일정과 도착 일정이 필요합니다.");
        }
        ...
        if (!itemIds.contains(command.fromItemId()) || !itemIds.contains(command.toItemId())) {
            throw EarthTripException.badRequest(
                    "ROUTE_OVERRIDE_ITEM_NOT_IN_DAY", "같은 날짜에 있는 일정 사이에서만 경로를 보정할 수 있습니다.");
        }
        int duration = command.durationMinutes() == null ? -1 : command.durationMinutes();
        if (duration < 0 || duration > 1_440) {
            throw EarthTripException.badRequest(
                    "INVALID_ROUTE_OVERRIDE_DURATION", "이동시간은 0~1440분이어야 합니다.");
        }
        if (command.distanceMeters() != null && command.distanceMeters() < 0) {
            throw EarthTripException.badRequest(
                    "INVALID_ROUTE_OVERRIDE_DISTANCE", "이동거리는 0 이상이어야 합니다.");
        }
    ...
    @Override
    public void delete(
            UUID tripId, UUID dayId, UUID overrideId, UUID actorUserId, long baseVersion) {
        days.requireDay(tripId, dayId, actorUserId);
        PlanningResourceUseCase.ResourceResult override =
                resources.get(tripId, actorUserId, "ROUTE_OVERRIDE", overrideId);
        if (!dayId.equals(override.parentId())) {
            throw EarthTripException.notFound("ROUTE_OVERRIDE_NOT_FOUND", "수동 경로 보정을 찾을 수 없습니다.");
        }
        resources.delete(
                tripId,
                actorUserId,
    ...
    private static String mode(String value) {
        ...
        if (!MODES.contains(normalized)) {
            throw EarthTripException.badRequest(
                    "INVALID_ROUTE_OVERRIDE_MODE", "지원하지 않는 수동 이동수단입니다.");
        }
        return normalized;
    }
    ...
    private static String note(String value) {
        ...
        if (normalized.length() > 1_000) {
            throw EarthTripException.badRequest(
                    "ROUTE_OVERRIDE_NOTE_TOO_LONG", "경로 보정 메모는 1000자 이하여야 합니다.");
        }
        return normalized;
    }
}
