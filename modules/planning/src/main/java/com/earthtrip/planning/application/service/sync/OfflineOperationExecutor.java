...
class OfflineOperationExecutor {
    ...
    ExecutionResult execute(
        ...
        long baseVersion = forcedBaseVersion == null
            ? command.baseVersion()
            : forcedBaseVersion;
        Map<String, Object> payload = forcedPayload == null
            ? command.payload()
            : forcedPayload;
        PlanningResourceUseCase.ResourceResult resource;
        switch (action) {
            case "CREATE" -> resource = resources.create(
                tripId, actorUserId, command.resourceType(),
                PlanningResourceUseCase.WritePermission.EDITOR,
                new PlanningResourceUseCase.ResourceCommand(
                    command.resourceId(), command.parentId(), command.localDate(), payload,
                    command.status(), command.sortOrder(), 0
                )
            );
            case "UPDATE" -> resource = resources.update(
                tripId, actorUserId, command.resourceType(), command.resourceId(),
                PlanningResourceUseCase.WritePermission.EDITOR,
                new PlanningResourceUseCase.ResourceCommand(
                    command.resourceId(), command.parentId(), command.localDate(), payload,
                    command.status(), command.sortOrder(), baseVersion
                )
            );
            case "RELOCATE" -> resource = resources.relocate(
                tripId, actorUserId, command.resourceType(), command.resourceId(),
                PlanningResourceUseCase.WritePermission.EDITOR, command.parentId(),
                command.localDate(), requiredSortOrder(command.sortOrder()), baseVersion
            );
            case "DELETE" -> {
                resources.delete(
                    tripId, actorUserId, command.resourceType(), command.resourceId(),
            ...
            case "PUT_USER_STATE" -> {
                PlanningResourceUseCase.UserStateResult state = resources.putUserState(
                    tripId, actorUserId, command.resourceType(), command.resourceId(),
                    requireStateType(command.stateType()), requiredStateValue(command.stateValue())
                );
                return new ExecutionResult(
                    command.resourceId(), Map.of(
    ...
    private static void requireCommand(OfflineOperationUseCase.OperationCommand command) {
        if (command.operationId() == null || command.action() == null
            || command.action().isBlank() || command.resourceType() == null
            || command.resourceType().isBlank() || command.resourceId() == null) {
    ...
    private static String requireStateType(String value) {
        if (value == null || value.isBlank()) {
            throw EarthTripException.badRequest(
                "STATE_TYPE_REQUIRED", "사용자 상태 유형이 필요합니다."
            );
        }
        return value;
    }
    ...
    private static Map<String, Object> requiredStateValue(Map<String, Object> value) {
        if (value == null) {
            throw EarthTripException.badRequest(
                "STATE_VALUE_REQUIRED", "사용자 상태 값이 필요합니다."
            );
        }
        return value;
    }
