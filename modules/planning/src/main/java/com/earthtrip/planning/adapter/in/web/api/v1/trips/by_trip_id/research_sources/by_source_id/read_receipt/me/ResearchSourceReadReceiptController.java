...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/research-sources/{sourceId}/read-receipt/me")
class ResearchSourceReadReceiptController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    ResearchSourceReadReceiptController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    void delete(@PathVariable UUID tripId, @PathVariable UUID sourceId) {
        useCase.deleteUserState(
                tripId, actor.requireUserId(), "RESEARCH_SOURCE", sourceId, "READ_RECEIPT");
    }
}
