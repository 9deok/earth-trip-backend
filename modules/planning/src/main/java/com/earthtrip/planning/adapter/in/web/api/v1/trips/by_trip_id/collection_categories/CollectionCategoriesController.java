...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/collection-categories")
class CollectionCategoriesController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    CollectionCategoriesController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    List<PlanningResourceUseCase.ResourceResult> get(@PathVariable UUID tripId) {
        return useCase.list(tripId, actor.requireUserId(), "COLLECTION_CATEGORY", null, null);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    PlanningResourceUseCase.ResourceResult post(
            @PathVariable UUID tripId, @Valid @RequestBody CategoryMutation r) {
        return useCase.create(
                ...
                new PlanningResourceUseCase.ResourceCommand(
                        r.requestId(), null, null, r.payload(), "ACTIVE", r.sortOrder(), 0));
    }
}
