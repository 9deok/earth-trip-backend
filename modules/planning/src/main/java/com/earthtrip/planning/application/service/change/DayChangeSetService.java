...
import com.earthtrip.planning.application.port.out.ScheduleSnapshotSerializationPort.SnapshotItem;
...
class DayChangeSetService implements DayChangeSetUseCase {
    ...
    private final ScheduleSnapshotSerializationPort snapshotSerialization;
    ...
    DayChangeSetService(
            ...
            ScheduleSnapshotSerializationPort snapshotSerialization,
            Clock clock) {
        this.days = days;
    ...
    public ChangeSetResult apply(
        ...
        return result(
                store.saveChangeSet(
                        new ScheduleChangeStorePort.ChangeSetRecord(
                                command.requestId(),
                                tripId,
                                dayId,
                                actorUserId,
    ...
    public ChangeSetResult revert(
            UUID tripId, UUID dayId, UUID changeSetId, UUID actorUserId, long baseVersion) {
        days.requireDay(tripId, dayId, actorUserId);
        ScheduleChangeStorePort.ChangeSetRecord changeSet =
                store.findChangeSet(changeSetId)
                        .orElseThrow(
                                () ->
                                        EarthTripException.notFound(
                                                "SCHEDULE_CHANGESET_NOT_FOUND",
                                                "일정 변경 이력을 찾을 수 없습니다."));
        ...
        Map<UUID, PlanningResourceUseCase.ResourceResult> currentById =
                current.stream()
                        .collect(
                        ...
                        .map(
                                item ->
                                        new ScheduleUseCase.OrderItem(
                                                item.itemId(),
                                                item.sortOrder(),
                                                currentById.get(item.itemId()).version()))
                        .toList());
        return result(
                store.saveChangeSet(
                        new ScheduleChangeStorePort.ChangeSetRecord(
    ...
    private static void validateOrder(
            List<PlanningResourceUseCase.ResourceResult> current, List<OrderItem> order) {
        if (order == null
    ...
    private static void requireUnchangedAfterApply(
            List<PlanningResourceUseCase.ResourceResult> current, List<SnapshotItem> after) {
        Map<UUID, PlanningResourceUseCase.ResourceResult> currentById =
                current.stream()
                        .collect(
                                java.util.stream.Collectors.toMap(
    ...
    private static List<SnapshotItem> snapshot(
        ...
        return resources.stream()
                .map(item -> new SnapshotItem(item.resourceId(), item.sortOrder(), item.version()))
    ...
    private String write(List<SnapshotItem> value) {
        return snapshotSerialization.serialize(value);
    }

    private List<SnapshotItem> read(String value) {
        return snapshotSerialization.deserialize(value);
    }

    private static void requireSameScope(
            ScheduleChangeStorePort.ChangeSetRecord record, UUID tripId, UUID dayId) {
    ...
    private static EarthTripException cannotRevert() {
        return EarthTripException.conflict(
                "SCHEDULE_CHANGED_AFTER_CHANGESET", "변경 적용 후 일정이 다시 수정되어 자동으로 되돌릴 수 없습니다.");
    }
