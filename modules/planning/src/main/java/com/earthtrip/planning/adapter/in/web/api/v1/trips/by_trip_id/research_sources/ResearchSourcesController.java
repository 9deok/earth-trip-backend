...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/research-sources")
class ResearchSourcesController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    ResearchSourcesController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    List<PlanningResourceUseCase.ResourceResult> get(@PathVariable UUID tripId) {
        return useCase.list(tripId, actor.requireUserId(), "RESEARCH_SOURCE", null, null);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    PlanningResourceUseCase.ResourceResult post(
            @PathVariable UUID tripId, @Valid @RequestBody SourceMutation r) {
