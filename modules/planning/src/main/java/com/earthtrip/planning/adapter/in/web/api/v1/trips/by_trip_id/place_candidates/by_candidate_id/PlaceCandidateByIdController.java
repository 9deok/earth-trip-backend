...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/place-candidates/{candidateId}")
class PlaceCandidateByIdController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    PlaceCandidateByIdController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    PlanningResourceUseCase.ResourceResult get(
        ...
        return useCase.get(tripId, actor.requireUserId(), "PLACE_CANDIDATE", candidateId);
    }
    ...
    PlanningResourceUseCase.ResourceResult patch(
            ...
            @Valid @RequestBody PlaceMutation r) {
    ...
    void delete(
            @PathVariable UUID tripId,
            @PathVariable UUID candidateId,
            @Valid @RequestBody PlaceDelete r) {
        useCase.delete(
                tripId,
                actor.requireUserId(),
                "PLACE_CANDIDATE",
...
record PlaceDelete(@Min(0) long baseVersion) {}
