...
class ActivityFeedService implements ActivityFeedUseCase {
    ...
    public ActivityPage activities(
        ...
        List<ActivityOperationStorePort.ActivityRecord> rows = activities.activities(
            tripId, cursor(after), pageSize + 1
        );
        boolean hasMore = rows.size() > pageSize;
        List<ActivityItem> items = rows.stream().limit(pageSize).map(this::item).toList();
        long nextCursor = items.isEmpty()
            ? cursor(after)
            : items.getLast().sequenceId();
    ...
    @Override
    @Transactional(readOnly = true)
    public ChangePage changes(
        UUID tripId,
        UUID actorUserId,
        Long after,
        Integer limit
    ) {
        ActivityPage page = activities(tripId, actorUserId, after, limit);
        return new ChangePage(page.items(), page.nextCursor(), page.hasMore());
    }

    @Override
    public ReadCursorResult updateReadCursor(
        UUID tripId,
        UUID actorUserId,
        long sequenceId
    ) {
        ...
        if (sequenceId < current || sequenceId > latest) {
            throw EarthTripException.badRequest(
                ...
                "읽음 위치는 현재 위치보다 작거나 최신 변경보다 클 수 없습니다."
            );
        }
        SyncStateStorePort.ReadCursorRecord saved = syncState.saveReadCursor(
    ...
    public RevertResult revert(
        ...
        ActivityOperationStorePort.ActivityRecord activity = activities
        ...
        if (!activity.action().equals("CREATED")) {
            throw EarthTripException.badRequest(
                "ACTIVITY_NOT_REVERSIBLE",
                "이 활동은 이전 값 스냅샷이 없어 자동으로 되돌릴 수 없습니다."
            );
        }
        resources.delete(
            tripId, actorUserId, activity.resourceType(), activity.resourceId(),
            PlanningResourceUseCase.WritePermission.EDITOR, resourceBaseVersion
        );
    ...
    private ActivityItem item(ActivityOperationStorePort.ActivityRecord record) {
        return new ActivityItem(
            record.sequenceId(), record.activityId(), record.actorId(), record.action(),
            record.resourceType(), record.resourceId(), record.details(),
            record.occurredAt(), record.action().equals("CREATED")
        );
    }
    ...
    private static long cursor(Long value) {
        ...
        if (value < 0) {
            throw EarthTripException.badRequest(
                "INVALID_CHANGE_CURSOR", "변경 커서는 0 이상이어야 합니다."
            );
        }
        return value;
    }
    ...
    private static int pageSize(Integer value) {
        ...
        if (value < 1 || value > 100) {
            throw EarthTripException.badRequest(
                "INVALID_PAGE_SIZE", "페이지 크기는 1~100이어야 합니다."
            );
        }
        return value;
    }
