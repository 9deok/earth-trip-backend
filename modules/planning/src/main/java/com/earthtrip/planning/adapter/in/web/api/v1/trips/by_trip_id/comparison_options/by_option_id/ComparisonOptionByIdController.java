...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/comparison-options/{optionId}")
class ComparisonOptionByIdController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    ComparisonOptionByIdController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    PlanningResourceUseCase.ResourceResult get(
            @PathVariable UUID tripId, @PathVariable UUID optionId) {
        return useCase.get(tripId, actor.requireUserId(), "COMPARISON_OPTION", optionId);
    }
    ...
    PlanningResourceUseCase.ResourceResult patch(
            ...
            @Valid @RequestBody ComparisonMutation r) {
    ...
    void delete(
            @PathVariable UUID tripId,
            @PathVariable UUID optionId,
            @Valid @RequestBody ComparisonDelete r) {
        useCase.delete(
                tripId,
                actor.requireUserId(),
                "COMPARISON_OPTION",
