...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/collection-categories/{categoryId}")
class CollectionCategoryByIdController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    CollectionCategoryByIdController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    PlanningResourceUseCase.ResourceResult get(
        ...
        return useCase.get(tripId, actor.requireUserId(), "COLLECTION_CATEGORY", categoryId);
    }
    ...
    PlanningResourceUseCase.ResourceResult patch(
            ...
            @Valid @RequestBody CategoryMutation r) {
    ...
    void delete(
            @PathVariable UUID tripId,
            @PathVariable UUID categoryId,
            @Valid @RequestBody CategoryDelete r) {
        useCase.delete(
                tripId,
                actor.requireUserId(),
                "COLLECTION_CATEGORY",
