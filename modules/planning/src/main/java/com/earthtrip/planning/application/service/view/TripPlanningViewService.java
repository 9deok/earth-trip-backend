...
class TripPlanningViewService implements TripPlanningView {
    ...
    private final ExecutionPlanningUseCase execution;
    private final PlanningResourceUseCase resources;

    TripPlanningViewService(
        TripDayUseCase days,
        ExecutionPlanningUseCase execution,
    ...
    public PlanningSnapshot snapshot(UUID tripId, UUID actorUserId) {
        ExecutionPlanningUseCase.TodayResult today = execution.today(tripId, actorUserId);
        return new PlanningSnapshot(
            days.list(tripId, actorUserId).stream()
                .map(day -> new Day(
                    day.dayId(), day.localDate(), day.dayNumber(), day.timeZone()
                ))
                .toList(),
            new Today(
                today.state(), today.localDate(), today.dayId(), today.timeZone(),
                today.items().stream()
                    .map(item -> new ScheduleItem(
                        item.resourceId(), item.payload(), item.status(),
    ...
    public List<SearchEntry> searchEntries(UUID tripId, UUID actorUserId) {
        return resources.listAll(tripId, actorUserId).stream()
            .map(resource -> new SearchEntry(
                resource.resourceId(), resource.resourceType(), resource.parentId(),
                resource.localDate(), resource.sortOrder(), resource.payload(), resource.status()
            ))
            .toList();
    }
}
