...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/decisions/{decisionId}/reopenings")
class DecisionReopeningsController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    DecisionReopeningsController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    PlanningResourceUseCase.ResourceResult post(
        ...
        PlanningResourceUseCase.ResourceResult old =
                useCase.get(tripId, actor.requireUserId(), "DECISION", decisionId);
        Map<String, Object> payload = new LinkedHashMap<>(old.payload());
        payload.put("reopeningReason", r.reason());
