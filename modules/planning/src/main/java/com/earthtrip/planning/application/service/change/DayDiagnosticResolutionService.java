...
class DayDiagnosticResolutionService implements DayDiagnosticResolutionUseCase {
    ...
    public ResolutionResult resolve(
        ...
        ScheduleChangeStorePort.ResolutionRecord saved = store.saveResolution(
            new ScheduleChangeStorePort.ResolutionRecord(
                diagnosticId, tripId, dayId, normalizedNote, actorUserId, clock.instant()
            )
        );
        return result(saved);
    }
    ...
    public void reopen(
        ...
        access.requireEditor(tripId, actorUserId);
        days.requireDay(tripId, dayId, actorUserId);
        ScheduleChangeStorePort.ResolutionRecord resolution = store
            .findResolution(diagnosticId)
            .filter(item -> item.tripId().equals(tripId) && item.dayId().equals(dayId))
            .orElseThrow(() -> EarthTripException.notFound(
                "DIAGNOSTIC_RESOLUTION_NOT_FOUND", "확인 처리된 일정 경고를 찾을 수 없습니다."
            ));
    ...
    private void requireCurrentDiagnostic(
        ...
        boolean exists = execution.diagnostics(tripId, dayId, actorUserId, null).stream()
