...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/comparison-options")
class ComparisonOptionsController {
    private final PlanningResourceUseCase useCase;
    private final CurrentActor actor;

    ComparisonOptionsController(PlanningResourceUseCase u, CurrentActor a) {
        useCase = u;
        actor = a;
    }
    ...
    List<PlanningResourceUseCase.ResourceResult> get(@PathVariable UUID tripId) {
        return useCase.list(tripId, actor.requireUserId(), "COMPARISON_OPTION", null, null);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    PlanningResourceUseCase.ResourceResult post(
            @PathVariable UUID tripId, @Valid @RequestBody ComparisonMutation r) {
