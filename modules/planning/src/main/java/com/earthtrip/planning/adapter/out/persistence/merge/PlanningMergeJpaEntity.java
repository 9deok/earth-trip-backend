...
class PlanningMergeJpaEntity {
    ...
    PlanningMergeJpaEntity(
        PlanningMergeStorePort.MergeRecord record,
        SerializedMerge serialized
    ) {
    ...
    String duplicateIds() { return duplicateIds; }
    String beforeSnapshot() { return beforeSnapshot; }
    String afterSnapshot() { return afterSnapshot; }
    String addedLinks() { return addedLinks; }
    ...
    PlanningMergeStorePort.MergeRecord toRecord(
        ...
        return new PlanningMergeStorePort.MergeRecord(
            UUID.fromString(id), UUID.fromString(tripId), resourceType,
            UUID.fromString(primaryId), duplicates, before, after, links, status,
            UUID.fromString(mergedBy), mergedAt,
            revertedBy == null ? null : UUID.fromString(revertedBy), revertedAt, version
        );
    }
