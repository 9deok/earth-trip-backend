...
class RoutePlanningService implements RoutePlanningUseCase {
    ...
    public RoutePreview routePreview(
            UUID tripId, UUID dayId, UUID actorUserId, Integer bufferMinutes) {
        List<Node> nodes = nodes(tripId, dayId, actorUserId);
        RoutePreferenceUseCase.PreferenceResult preference = preferences.get(tripId, actorUserId);
        ...
        Map<LegKey, RouteOverrideUseCase.OverrideResult> overrides =
                overrides(tripId, dayId, actorUserId);
    ...
    public List<DayRouteSummary> tripRoutePreview(
            UUID tripId, UUID actorUserId, Integer bufferMinutes) {
        return days.list(tripId, actorUserId).stream()
                .map(
                        day -> {
                            RoutePreview preview =
                                    routePreview(tripId, day.dayId(), actorUserId, bufferMinutes);
                            return new DayRouteSummary(
                                    day.dayId(),
                                    day.localDate(),
                                    preview.currentDistanceMeters(),
                                    preview.currentTravelMinutes(),
                                    preview.currentOrder());
                        })
                .toList();
    }
    ...
    public RoutePreview replan(
            UUID tripId, UUID dayId, UUID actorUserId, Integer delayMinutes, boolean rain) {
        ...
        List<Node> candidates =
                rain
                        ? original.stream()
                                .filter(
                                        node ->
                                                Boolean.TRUE.equals(node.payload().get("indoor"))
                                                        || !Boolean.TRUE.equals(
                                                                node.payload()
                                                                        .get("weatherSensitive")))
                                .toList()
                        : original;
        ...
        if (delay > 0) {
            warnings.add(
                    informationalDiagnostic(
                            dayId,
                            "DELAY_APPLIED",
                            "지연 시간을 반영한 재계획 미리보기입니다.",
                            Map.of("delayMinutes", delay)));
        }
        if (rain) {
            warnings.add(
                    informationalDiagnostic(
                            dayId, "RAIN_FILTER_APPLIED", "비에 민감한 야외 일정을 제외한 후보입니다.", Map.of()));
        }
        return new RoutePreview(
                preview.dayId(),
                preview.currentOrder(),
                preview.recommendedOrder(),
                preview.currentDistanceMeters(),
                preview.recommendedDistanceMeters(),
                preview.currentTravelMinutes() + delay,
                preview.recommendedTravelMinutes() + delay,
                preview.bufferMinutes(),
                preview.source(),
    ...
    public List<RouteLeg> routeLegs(
        ...
        for (int index = 1; index < nodes.size(); index++) {
            ...
            RouteOverrideUseCase.OverrideResult override =
                    overrides.get(new LegKey(from.id(), to.id()));
            if (override == null && (!hasLocation(from) || !hasLocation(to))) {
                continue;
            }
            long meters =
                    override != null && override.distanceMeters() != null
    ...
    private RoutePreview preview(
        ...
        return new RoutePreview(
                ...
                bufferMinutes,
                "LOCAL_HEURISTIC",
                diagnostics(tripId, dayId, nodes, bufferMinutes, current, best, overrides));
    }
    ...
    private List<Diagnostic> diagnostics(
        ...
        return raw.stream().map(item -> diagnostic(dayId, item, resolutions)).toList();
    }
    ...
    private static void addReachabilityDiagnostic(
            ...
            Map<LegKey, RouteOverrideUseCase.OverrideResult> overrides) {
        RouteOverrideUseCase.OverrideResult override =
                overrides.get(new LegKey(previous.id(), next.id()));
    ...
    private static int normalizeBuffer(Integer bufferMinutes, int defaultValue) {
        return bufferMinutes == null ? defaultValue : Math.max(0, Math.min(120, bufferMinutes));
    }

    private Map<LegKey, RouteOverrideUseCase.OverrideResult> overrides(
            UUID tripId, UUID dayId, UUID actorUserId) {
    ...
    private static Diagnostic informationalDiagnostic(
            UUID dayId, String code, String message, Map<String, Object> details) {
        return new Diagnostic(
                diagnosticId(dayId, code, null),
                code,
                "INFO",
                null,
                message,
