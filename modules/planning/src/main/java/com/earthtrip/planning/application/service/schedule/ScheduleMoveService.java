...
class ScheduleMoveService implements ScheduleMoveUseCase {
    ...
    public MoveResult move(UUID tripId, UUID actorUserId, MoveCommand command) {
        if (command == null
                || command.itemId() == null
                || command.sourceDayId() == null
                || command.targetDayId() == null) {
            throw EarthTripException.badRequest(
                    "INVALID_SCHEDULE_MOVE", "이동할 일정과 출발·도착 날짜가 필요합니다.");
        }
        TripDayUseCase.DayResult sourceDay =
                days.requireDay(tripId, command.sourceDayId(), actorUserId);
        TripDayUseCase.DayResult targetDay =
                days.requireDay(tripId, command.targetDayId(), actorUserId);
        List<PlanningResourceUseCase.ResourceResult> source =
        ...
        if (sourceDay.dayId().equals(targetDay.dayId())) {
            applyOrder(
                    ...
                    sourceDay,
                    source,
                    normalizeOrder(command.targetOrder()),
                    null);
            List<PlanningResourceUseCase.ResourceResult> ordered =
                    schedule.list(tripId, sourceDay.dayId(), actorUserId);
        ...
        applyTargetOrder(tripId, actorUserId, targetDay, target, targetOrder, moved);
        return new MoveResult(
                moved.resourceId(),
                sourceDay.dayId(),
                targetDay.dayId(),
    ...
    private void applyTargetOrder(
            ...
            PlanningResourceUseCase.ResourceResult moved) {
        Map<UUID, PlanningResourceUseCase.ResourceResult> current =
                target.stream()
                        .collect(
                                Collectors.toMap(
    ...
    private static List<OrderItem> normalizeOrder(List<OrderItem> order) {
        if (order == null) {
            throw EarthTripException.badRequest("SCHEDULE_ORDER_REQUIRED", "변경 후 일정 순서가 필요합니다.");
        }
        ...
        if (safe.stream().anyMatch(java.util.Objects::isNull)
            ...
            throw EarthTripException.badRequest(
                    "INVALID_SCHEDULE_ORDER", "일정과 정렬 순서를 중복 없이 입력해 주세요.");
        }
        return safe;
    }

    private static void requireExactIds(List<OrderItem> order, Set<UUID> expected, String scope) {
        Set<UUID> actual = order.stream().map(OrderItem::itemId).collect(Collectors.toSet());
        if (!actual.equals(expected)) {
            throw EarthTripException.badRequest(
                    "INVALID_SCHEDULE_ORDER", scope + "의 모든 일정을 정확히 한 번 포함해야 합니다.");
        }
    }

    private static void requireVersion(
            PlanningResourceUseCase.ResourceResult item, long baseVersion) {
