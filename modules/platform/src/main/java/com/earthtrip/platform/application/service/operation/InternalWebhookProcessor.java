...
class InternalWebhookProcessor {
    ...
    private void processObjectStorage(Map<String, Object> payload) {
        ...
        FileStorePort.UploadRecord upload = files.upload(uploadSessionId).orElseThrow(() ->
        ...
        if (checksum != null && !file.checksum().equalsIgnoreCase(checksum)) {
            throw EarthTripException.conflict(
                ...
                "업로드 완료 이벤트의 체크섬이 파일과 일치하지 않습니다."
            );
        }
        Instant now = clock.instant();
        files.saveUpload(new FileStorePort.UploadRecord(
            upload.id(),
            upload.fileId(),
            "COMPLETED",
            upload.expiresAt(),
    ...
    private void processCalendar(Map<String, Object> payload) {
        IntegrationStorePort.ConnectionRecord connection = loadConnection(
            requiredUuid(payload, "connectionId")
        );
        ...
        IntegrationStorePort.CalendarRecord calendar = integrations.calendar(tripId)
        ...
        if (!calendar.connectionId().equals(connection.id())) {
            throw EarthTripException.conflict(
                "CALENDAR_CONNECTION_MISMATCH",
                "캘린더 이벤트의 연결이 여행 설정과 일치하지 않습니다."
            );
        }
        integrations.saveCalendar(new IntegrationStorePort.CalendarRecord(
            calendar.tripId(),
    ...
    private void processFinancialProvider(Map<String, Object> payload) {
        ...
        if (!connection.kind().equals("FINANCIAL")) {
            throw EarthTripException.conflict(
                "FINANCIAL_CONNECTION_REQUIRED",
                "금융 제공자 이벤트가 금융 연결을 가리키지 않습니다."
            );
        }
        integrations.saveConnection(copyConnection(
            connection,
    ...
    private IntegrationStorePort.ConnectionRecord copyConnection(
        IntegrationStorePort.ConnectionRecord current,
        String status,
        Instant now
    ) {
        String errorCode = status.equals("ACTIVE")
            ? null
    ...
    private FileStorePort.FileRecord loadFile(UUID id) {
        return files.file(id).filter(file -> file.deletedAt() == null).orElseThrow(() ->
            EarthTripException.notFound("FILE_NOT_FOUND", "파일을 찾을 수 없습니다.")
        );
    }

    private IntegrationStorePort.ConnectionRecord loadConnection(UUID id) {
        return integrations.connection(id).filter(connection -> connection.revokedAt() == null)
            .orElseThrow(() -> EarthTripException.notFound(
    ...
    private static String providerStatus(String value) {
        ...
        if (!Set.of("ACTIVE", "REAUTHORIZATION_REQUIRED", "ERROR", "REVOKED")
            .contains(status)) {
            throw EarthTripException.badRequest(
                "INVALID_PROVIDER_STATUS",
                "지원하지 않는 외부 제공자 상태입니다."
            );
        }
        return status;
    }
