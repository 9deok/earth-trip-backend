...
public interface ExternalTravelProviderPort {
    List<ExternalTravelUseCase.GeoResult> forward(String query, String language, int limit);

    ExternalTravelUseCase.GeoResult reverse(BigDecimal lat, BigDecimal lon, String language);

    ExternalTravelUseCase.PlaceUrlResult resolvePlaceUrl(String url, String language);

    ExternalTravelUseCase.TimeZoneResult timeZone(BigDecimal lat, BigDecimal lon);

    List<ExternalTravelUseCase.TransportStatusResult> transportStatuses(
