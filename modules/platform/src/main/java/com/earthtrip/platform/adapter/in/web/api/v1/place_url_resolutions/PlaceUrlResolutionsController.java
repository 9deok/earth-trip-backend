...
@RestController
@RequestMapping("/api/v1/place-url-resolutions")
class PlaceUrlResolutionsController {
    private final ExternalTravelUseCase u;

    PlaceUrlResolutionsController(ExternalTravelUseCase u) {
        this.u = u;
    }

    @PostMapping
    ExternalTravelUseCase.PlaceUrlResult post(@Valid @RequestBody PlaceUrlRequest r) {
        return u.resolvePlaceUrl(r.url(), r.language());
    }
}
