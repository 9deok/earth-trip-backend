...
class TripExportRenderer {
    ...
    private final StructuredJsonPort structuredJson;
    private final PdfDocumentRendererPort pdfRenderer;
    ...
    TripExportRenderer(
            ...
            StructuredJsonPort structuredJson,
            PdfDocumentRendererPort pdfRenderer) {
    ...
    RenderedArtifact render(UUID tripId, UUID actorUserId, String format, Set<String> scopes) {
        ...
        return switch (format) {
            ...
            case "KML" ->
                    artifact(bundle, "kml", "application/vnd.google-earth.kml+xml", kml(bundle));
    ...
    private ExportBundle bundle(UUID tripId, UUID actorUserId, Set<String> scopes) {
        ...
        List<TripPlanningView.SearchEntry> planningEntries =
                scopes.contains("PLANNING")
                        ? planning.searchEntries(tripId, actorUserId)
                        : List.of();
        TripWalletView.WalletSnapshot walletSnapshot =
                scopes.contains("WALLET") ? wallet.snapshot(tripId, actorUserId) : null;
        List<TripExpenseView.Entry> expenseEntries =
                scopes.contains("EXPENSE")
                        ? expenses.searchEntries(tripId, actorUserId)
                        : List.of();
        return new ExportBundle(
                structure.trip(),
                scopes.contains("STRUCTURE") ? structure.segments() : List.of(),
                planningEntries,
                walletSnapshot,
                expenseEntries);
    }
    ...
    private byte[] json(ExportBundle bundle) {
        try {
            return structuredJson.serializePretty(bundle);
    ...
    private static byte[] csv(ExportBundle bundle) {
        StringBuilder value =
                new StringBuilder("\uFEFFsection,type,title,date,amount,currency,status\r\n");
        for (TripPlanningView.SearchEntry entry : bundle.planning()) {
            value.append("planning,")
                    .append(csvValue(entry.type()))
                    .append(',')
                    .append(csvValue(title(entry.payload())))
                    .append(',')
                    .append(csvValue(entry.localDate() == null ? "" : entry.localDate().toString()))
                    .append(",,,")
                    .append(csvValue(entry.status()))
                    .append("\r\n");
        }
        for (TripExpenseView.Entry entry : bundle.expenses()) {
            value.append("expense,")
                    .append(csvValue(entry.categoryCode()))
                    .append(',')
                    .append(csvValue(entry.title()))
                    .append(',')
                    .append(csvValue(entry.occurredAt().toString()))
                    .append(',')
                    .append(entry.amountMinor())
                    .append(',')
                    .append(csvValue(entry.currency()))
                    .append(',')
                    .append(csvValue(entry.status()))
                    .append("\r\n");
        }
    ...
    private static byte[] ics(ExportBundle bundle) {
        ...
        for (TripPlanningView.SearchEntry entry : bundle.planning()) {
            ...
            value.append("BEGIN:VEVENT\r\nUID:")
                    ...
                    .append("@earthtrip\r\n")
                    .append("DTSTAMP:")
                    .append(ICS_TIME.format(Instant.now()))
                    .append("\r\n")
                    .append("DTSTART:")
                    .append(ICS_TIME.format(start))
                    .append("\r\n");
            if (end != null) {
                value.append("DTEND:").append(ICS_TIME.format(end)).append("\r\n");
            }
            value.append("SUMMARY:")
                    .append(icsEscape(title(entry.payload())))
                    .append("\r\nEND:VEVENT\r\n");
        }
    ...
    private static byte[] kml(ExportBundle bundle) {
        ...
        for (TripStructureView.Segment segment : bundle.segments()) {
            ...
            value.append("<Placemark><name>")
                    .append(xml(segment.cityName() == null ? segment.type() : segment.cityName()))
                    .append("</name><Point><coordinates>")
                    .append(segment.longitude())
                    .append(',')
                    .append(segment.latitude())
        ...
        for (TripPlanningView.SearchEntry entry : bundle.planning()) {
            ...
            value.append("<Placemark><name>")
                    .append(xml(title(entry.payload())))
                    .append("</name><Point><coordinates>")
                    .append(longitude)
                    .append(',')
                    .append(latitude)
        ...
        lines.add(
                TextLine.body(
                        "도시·이동 "
                                ...
                                + "개  ·  계획 "
                                + bundle.planning().size()
                                + "개  ·  지출 "
                                + bundle.expenses().size()
                                + "건"));
    ...
    private static void appendSegments(
            List<TextLine> lines, List<TripStructureView.Segment> segments) {
        if (segments.isEmpty()) {
            return;
        }
        ...
        for (TripStructureView.Segment segment : segments) {
            ...
            String period =
                    segment.startDate() == null && segment.endDate() == null
                            ? ""
                            : "  ·  "
    ...
    private static void appendPlanning(
            List<TextLine> lines, List<TripPlanningView.SearchEntry> entries) {
        if (entries.isEmpty()) {
            return;
        }
        ...
        for (TripPlanningView.SearchEntry entry : entries) {
            ...
            lines.add(TextLine.caption("    " + entry.type() + "  ·  상태 " + value(entry.status())));
        }
    }

    private static void appendWallet(List<TextLine> lines, TripWalletView.WalletSnapshot snapshot) {
        if (snapshot == null) {
            return;
        }
        ...
        lines.add(
                TextLine.body(
                        "예약 "
                                ...
                                + "건  ·  오프라인 준비 "
                                + snapshot.offlineReadyCount()
                                + "건  ·  확인 필요 "
                                + snapshot.actionRequiredCount()
                                + "건"));
    ...
    private static void appendExpenses(List<TextLine> lines, List<TripExpenseView.Entry> entries) {
        if (entries.isEmpty()) {
            return;
        }
    ...
    private static String amount(long amountMinor, String currencyCode) {
        try {
            ...
            BigDecimal amount =
                    BigDecimal.valueOf(amountMinor, fractionDigits)
    ...
    private static RenderedArtifact artifact(
            ExportBundle bundle, String extension, String mimeType, byte[] content) {
