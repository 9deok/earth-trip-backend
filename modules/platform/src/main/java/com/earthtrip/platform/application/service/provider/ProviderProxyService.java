...
class ProviderProxyService implements ProviderProxyUseCase {
    ...
    public List<PlaceSummary> searchPlaces(
        ...
        if (safeLimit < 1 || safeLimit > 20) {
            throw EarthTripException.badRequest(
                    "INVALID_PLACE_SEARCH_LIMIT", "장소 검색 개수는 1에서 20 사이여야 합니다.");
        }
        return places.search(safeQuery, safeLanguage, nearLatitude, nearLongitude, safeLimit);
    }
    ...
    public RouteResult route(RouteQuery query) {
        ...
        if (waypoints.size() > 10) {
            throw EarthTripException.badRequest("TOO_MANY_ROUTE_WAYPOINTS", "경유지는 10개 이하여야 합니다.");
        }
    ...
    public MatrixResult routeMatrix(MatrixQuery query) {
        if (query == null
                || query.origins() == null
                || query.destinations() == null
                || query.origins().isEmpty()
                || query.destinations().isEmpty()) {
            throw EarthTripException.badRequest(
                    "ROUTE_MATRIX_POINTS_REQUIRED", "출발지와 도착지가 각각 하나 이상 필요합니다.");
        }
        if ((long) query.origins().size() * query.destinations().size() > 100) {
            throw EarthTripException.badRequest(
                    "ROUTE_MATRIX_TOO_LARGE", "경로 행렬은 출발지×도착지 100개 이하여야 합니다.");
        }
    ...
    public List<WeatherDay> weather(WeatherQuery query) {
        if (query == null || query.startDate() == null || query.endDate() == null) {
            throw EarthTripException.badRequest("WEATHER_QUERY_REQUIRED", "위치와 예보 날짜가 필요합니다.");
        }
        validateCoordinates(query.latitude(), query.longitude());
        long days = ChronoUnit.DAYS.between(query.startDate(), query.endDate());
        if (days < 0 || days > 9) {
            throw EarthTripException.badRequest(
                    "INVALID_WEATHER_RANGE", "날씨 예보 범위는 시작일과 종료일을 포함해 최대 10일입니다.");
        }
        String timeZone = text(query.timeZone(), "TIME_ZONE_REQUIRED", "IANA 시간대가 필요합니다.", 80);
        try {
            java.time.ZoneId.of(timeZone);
        } catch (java.time.DateTimeException exception) {
            throw EarthTripException.badRequest("INVALID_TIME_ZONE", "유효한 IANA 시간대가 아닙니다.");
        }
    ...
    public ExchangeRateResult exchangeRates(
        ...
        if (quoteCurrencies == null || quoteCurrencies.isEmpty() || quoteCurrencies.size() > 20) {
            throw EarthTripException.badRequest(
                    "INVALID_QUOTE_CURRENCIES", "상대 통화는 1개 이상 20개 이하로 입력해 주세요.");
        }
        List<String> quotes =
                quoteCurrencies.stream().map(ProviderProxyService::currency).distinct().toList();
        if (quotes.contains(base)) {
            throw EarthTripException.badRequest(
                    "SAME_EXCHANGE_CURRENCY", "기준 통화는 상대 통화 목록에 포함할 수 없습니다.");
        }
    ...
    private static void validatePoint(RoutePoint point) {
        ...
        boolean hasPlaceId = point.providerPlaceId() != null && !point.providerPlaceId().isBlank();
        boolean hasCoordinates = point.latitude() != null || point.longitude() != null;
        if (hasPlaceId == hasCoordinates) {
            throw EarthTripException.badRequest(
                    "INVALID_ROUTE_POINT", "경로 지점은 장소 ID 또는 좌표 중 하나만 사용해야 합니다.");
        }
        if (hasCoordinates) {
    ...
    private static void validateCoordinates(BigDecimal latitude, BigDecimal longitude) {
        if (latitude == null
                ...
                || longitude.compareTo(BigDecimal.valueOf(180)) > 0) {
            throw EarthTripException.badRequest(
                    "INVALID_COORDINATES", "위도는 -90~90, 경도는 -180~180 사이여야 합니다.");
        }
    }
    ...
    private static String language(String value) {
        ...
        if (!normalized.matches("[A-Za-z]{2,3}(-[A-Za-z0-9]{2,8})?")) {
            throw EarthTripException.badRequest("INVALID_LANGUAGE", "언어 태그 형식을 확인해 주세요.");
        }
        return normalized;
    }
    ...
    private static String safeExternalUrl(String value) {
        try {
            URI uri = new URI(text(value, "URL_REQUIRED", "미리 볼 URL이 필요합니다.", 2_048));
            ...
            if ((!scheme.equals("http") && !scheme.equals("https"))
                ...
                throw EarthTripException.badRequest(
                        "UNSAFE_PREVIEW_URL", "외부 HTTP(S) URL만 미리 볼 수 있습니다.");
            }
    ...
    private static String text(String value, String code, String message, int maximumLength) {
