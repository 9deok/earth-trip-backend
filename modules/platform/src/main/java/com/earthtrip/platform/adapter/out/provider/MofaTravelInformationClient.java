...
class MofaTravelInformationClient {
    ...
    List<ExternalTravelUseCase.InformationResult> emergency(
        ...
        for (String countryCode : countryCodes) {
            ...
            for (JsonNode item : items(get(uri))) {
                result.add(
                        new ExternalTravelUseCase.InformationResult(
                                first(
                                        item,
                                        "country_iso_alp2",
                                        "countryIsoAlp2",
    ...
    private JsonNode get(URI uri) {
        try {
            ...
            throw EarthTripException.unavailable(
                    "MOFA_PROVIDER_UNAVAILABLE", "외교부 여행정보 제공자에 연결할 수 없습니다.");
        }
    }
    ...
    private static String first(
            JsonNode item, String first, String second, String third, String fallback) {
    ...
    private void requireConfigured() {
        if (!configured()) {
            throw EarthTripException.unavailable(
                    "MOFA_PROVIDER_NOT_CONFIGURED", "외교부 공공데이터 서비스 키가 설정되지 않았습니다.");
        }
    }

    private static EarthTripException invalidResponse() {
        return new EarthTripException(
                "INVALID_MOFA_PROVIDER_RESPONSE", 502, "외교부 여행정보 응답을 해석할 수 없습니다.");
    }
}
