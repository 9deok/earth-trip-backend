...
class InternalOperationsService implements InternalOperationsUseCase {
    ...
    public WebhookResult acceptWebhook(
        ...
        if (body.getBytes(java.nio.charset.StandardCharsets.UTF_8).length > MAX_WEBHOOK_BYTES) {
            throw new EarthTripException(
                    "WEBHOOK_PAYLOAD_TOO_LARGE", 413, "웹훅 본문은 1MB를 초과할 수 없습니다.");
        }
        WebhookSecurityPort.VerifiedWebhook verified =
                webhookSecurity.verify(provider, eventId, timestamp, signature, body);
        OperationalStorePort.WebhookReceiptRecord existing =
                store.webhookReceipt(verified.provider(), verified.eventId()).orElse(null);
        if (existing != null) {
            if (!existing.payloadDigest().equals(verified.payloadDigest())) {
                throw EarthTripException.conflict(
                        "WEBHOOK_EVENT_ID_REUSED", "같은 이벤트 ID가 다른 본문과 함께 다시 사용되었습니다.");
            }
        ...
        OperationalStorePort.JobRecord queued =
                store.saveJob(
                        new OperationalStorePort.JobRecord(
                                jobId,
    ...
    public JobResult retryJob(UUID jobId) {
        ...
        if (current.status().equals("SUCCEEDED")) {
            throw EarthTripException.conflict(
                    "OPERATIONAL_JOB_ALREADY_SUCCEEDED", "이미 성공한 운영 작업은 재시도할 수 없습니다.");
        }
        if (current.status().equals("RUNNING")) {
            throw EarthTripException.conflict(
                    "OPERATIONAL_JOB_RUNNING", "실행 중인 운영 작업은 재시도할 수 없습니다.");
        }
        Instant now = clock.instant();
        OperationalStorePort.JobRecord queued =
                store.saveJob(
    ...
    public List<DeadLetterResult> deadLetters(String status, int limit) {
        String normalized =
                optionalEnum(status, DEAD_LETTER_STATUSES, "INVALID_DEAD_LETTER_STATUS");
    ...
    public JobResult replayDeadLetter(UUID eventId) {
        OperationalStorePort.DeadLetterRecord event =
                store.deadLetter(eventId)
                        .orElseThrow(
                                () ->
                                        EarthTripException.notFound(
                                                "DEAD_LETTER_NOT_FOUND", "데드레터 이벤트를 찾을 수 없습니다."));
        if (!event.status().equals("OPEN")) {
            throw EarthTripException.conflict(
                    "DEAD_LETTER_ALREADY_REPLAYED", "이미 재처리된 데드레터 이벤트입니다.");
        }
        JobResult retried = retryJob(event.jobId());
        if (retried.status().equals("SUCCEEDED")) {
            store.saveDeadLetter(
                    new OperationalStorePort.DeadLetterRecord(
                            event.id(),
                            event.jobId(),
                            event.eventType(),
                            event.payload(),
    ...
    public List<AuditResult> auditEvents(
        ...
        return store
                .auditEvents(
                        ...
                        validateLimit(limit))
                .stream()
                .map(InternalOperationResultMapper::audit)
                .toList();
    }

    private OperationalStorePort.JobRecord process(
            OperationalStorePort.JobRecord queued, String provider) {
        Instant startedAt = clock.instant();
        OperationalStorePort.JobRecord running =
                store.saveJob(copyJob(queued, "RUNNING", startedAt, null, null, null));
        try {
            webhookProcessor.process(provider, running);
            Instant completedAt = clock.instant();
            OperationalStorePort.JobRecord succeeded =
                    store.saveJob(
                            copyJob(running, "SUCCEEDED", completedAt, completedAt, null, null));
            audit(
                    "PROVIDER",
                    provider,
                    "WEBHOOK_PROCESSED",
            ...
            String code =
                    exception instanceof EarthTripException earthTrip
                            ? earthTrip.code()
                            : "INTERNAL_WEBHOOK_PROCESSING_FAILED";
            String message = clip(exception.getMessage(), 500);
            Instant failedAt = clock.instant();
            OperationalStorePort.JobRecord failed =
                    store.saveJob(copyJob(running, "FAILED", failedAt, failedAt, code, message));
            OperationalStorePort.DeadLetterRecord old =
                    store.openDeadLetterForJob(failed.id()).orElse(null);
            store.saveDeadLetter(
                    new OperationalStorePort.DeadLetterRecord(
    ...
    private OperationalStorePort.JobRecord loadJob(UUID id) {
        return store.job(id)
                .orElseThrow(
                        () ->
                                EarthTripException.notFound(
                                        "OPERATIONAL_JOB_NOT_FOUND", "운영 작업을 찾을 수 없습니다."));
    }
    ...
    private void audit(
        ...
        store.saveAudit(
                new OperationalStorePort.AuditRecord(
                        null,
                        UUID.randomUUID(),
                        actorType,
                        actorId,
    ...
    private Map<String, Object> readBody(String rawBody) {
        ...
        try {
            Map<String, Object> parsed = structuredJson.deserializeObject(rawBody);
            if (parsed == null) {
                return Map.of();
            }
            return Collections.unmodifiableMap(new LinkedHashMap<>(parsed));
        } catch (IllegalArgumentException exception) {
            throw EarthTripException.badRequest("INVALID_WEBHOOK_BODY", "웹훅 본문은 JSON 객체여야 합니다.");
        }
    }
    ...
    private static int validateLimit(int limit) {
        if (limit < 1 || limit > 200) {
            throw EarthTripException.badRequest("INVALID_PAGE_LIMIT", "limit은 1에서 200 사이여야 합니다.");
        }
        return limit;
    }
    ...
    private static String providerFromJobType(String jobType) {
        if (!jobType.endsWith("_WEBHOOK")) {
            throw EarthTripException.badRequest(
                    "UNSUPPORTED_OPERATIONAL_JOB", "웹훅으로 생성되지 않은 작업은 이 처리기로 재시도할 수 없습니다.");
        }
    ...
    private static String clip(String value, int maxLength) {
        String normalized = value == null || value.isBlank() ? "알 수 없는 처리 오류" : value.strip();
