...
class GoogleCalendarProviderAdapterTest {
    ...
    void exchangesAuthorizationCodeAndEncryptsRefreshToken() {
        ...
        var result = adapter.authorize(new ExternalAccountProviderPort.AuthorizationCommand(
            "authorization-code",
    ...
    void createsDedicatedCalendarAndIdempotentEarthTripEvent() {
        ...
        server.expect(requestTo(
            ...
            .andExpect(jsonPath("$.extendedProperties.private.earthTripTripId")
                .value(tripId.toString()))
            .andRespond(withSuccess("{\"id\":\"" + eventId + "\"}", MediaType.APPLICATION_JSON));
        ...
        var result = adapter.syncCalendar(new ExternalAccountProviderPort.CalendarSyncCommand(
            tripId,
            "서울 여행",
            "Asia/Seoul",
            Map.of("_earthTripGoogleRefreshToken", "protected:refresh"),
            Map.of(),
            List.of(new ExternalAccountProviderPort.CalendarEvent(
                itemId,
    ...
    private static GoogleCalendarProviderAdapter adapter(RestClient.Builder builder) {
        IntegrationSecretProtectorPort protector = new IntegrationSecretProtectorPort() {
            @Override
