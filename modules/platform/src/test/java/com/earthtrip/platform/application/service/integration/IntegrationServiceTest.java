...
class IntegrationServiceTest {
    ...
    void verifiesActiveConnectionAndPersistsSuccessfulSyncJob() {
        ...
        when(fixture.provider().checkConnection(any())).thenReturn(
            new ExternalAccountProviderPort.ConnectionCheckResult(
                "ACTIVE", Map.of("tokenRefresh", "SUCCEEDED")
            )
        );

        IntegrationUseCase.SyncJobResult result = fixture.service().syncConnection(
            fixture.userId(), fixture.connectionId(), UUID.randomUUID(), Map.of()
        );
    ...
    void marksConnectionForReauthorizationWhenRefreshGrantIsRejected() {
        Fixture fixture = fixture();
        when(fixture.provider().configured()).thenReturn(true);
        when(fixture.provider().checkConnection(any())).thenThrow(
        ...
        verify(fixture.store()).saveConnection(org.mockito.ArgumentMatchers.argThat(
            connection -> connection.status().equals("REAUTHORIZATION_REQUIRED")
                && connection.errorCode().equals("GOOGLE_CALENDAR_REAUTHORIZATION_REQUIRED")
        ));
    }
    ...
    void completesPendingOAuthConnectionWithSameIdempotencyKey() {
        ...
        IntegrationStorePort.ConnectionRecord pending = new IntegrationStorePort.ConnectionRecord(
            requestId,
            userId,
            "GENERAL",
            "GOOGLE_CALENDAR",
        ...
        when(provider.authorize(any())).thenReturn(
        ...
        IntegrationUseCase.ConnectionResult result = service.createConnection(
            userId,
            "GENERAL",
        ...
        assertThat(result.metadata()).containsEntry("displayName", "Google Calendar")
