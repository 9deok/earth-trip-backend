...
class ExpenseCategoryService implements ExpenseCategoryUseCase {
    ...
    private static final List<SystemCategory> SYSTEM_CATEGORIES =
            List.of(
    ...
    public List<CategoryResult> list(UUID tripId, UUID actorUserId) {
        ...
        List<CategoryResult> results =
                new java.util.ArrayList<>(
                        SYSTEM_CATEGORIES.stream()
                                .map(category -> result(tripId, category, now))
                                .toList());
        results.addAll(
                categories.findAll(tripId).stream().map(ExpenseCategoryService::result).toList());
        return results.stream()
                .sorted(
                        Comparator.comparingInt(CategoryResult::sortOrder)
    ...
    public CategoryResult create(
        ...
        requireUniqueName(tripId, safeName, null);
        String safeColor = color(color);
        ExpenseCategoryStorePort.CategoryRecord existing =
                categories.findById(requestId).orElse(null);
        if (existing != null) {
        ...
        return result(
                categories.save(
                        new ExpenseCategoryStorePort.CategoryRecord(
                                requestId,
                                tripId,
                                code,
                                safeName,
    ...
    public CategoryResult update(
        ...
        String nextName = name == null ? current.name() : name(name);
        requireUniqueName(tripId, nextName, current.id());
        Instant now = clock.instant();
        return result(
                categories.save(
                        new ExpenseCategoryStorePort.CategoryRecord(
    ...
    public void delete(
        ...
        ExpenseCategoryStorePort.CategoryRecord current = loadCustom(tripId, categoryId);
        requireVersion(current, baseVersion);
        List<Expense> affected =
                expenses.findAll(tripId).stream()
                        .filter(expense -> expense.categoryCode().equals(current.code()))
                        .toList();
        String replacement =
                replacementCode == null ? null : requireCategoryCode(tripId, replacementCode);
        if (!affected.isEmpty() && replacement == null) {
            throw EarthTripException.conflict(
                    "EXPENSE_CATEGORY_IN_USE", "이 분류를 사용하는 지출이 있습니다. 대체 분류를 지정해 주세요.");
        }
        if (current.code().equals(replacement)) {
            throw EarthTripException.badRequest(
                    "SAME_REPLACEMENT_CATEGORY", "삭제할 분류와 대체 분류는 달라야 합니다.");
        }
        Instant now = clock.instant();
                ...
                .orElseThrow(
                        () ->
                                EarthTripException.notFound(
                                        "EXPENSE_CATEGORY_NOT_FOUND", "지출 분류를 찾을 수 없습니다."));
    }
    ...
    private void requireUniqueName(UUID tripId, String name, UUID excludedCategoryId) {
        boolean systemDuplicate =
                SYSTEM_CATEGORIES.stream()
                        .anyMatch(category -> category.name().equalsIgnoreCase(name));
        boolean customDuplicate =
                categories.findAll(tripId).stream()
                        .anyMatch(
                                category ->
                                        !category.id().equals(excludedCategoryId)
                                                && category.name().equalsIgnoreCase(name));
        if (systemDuplicate || customDuplicate) {
            throw EarthTripException.conflict(
                    "EXPENSE_CATEGORY_NAME_DUPLICATE", "이미 같은 이름의 지출 분류가 있습니다.");
        }
    }
    ...
    private static void rejectSystemMutation(UUID categoryId) {
        if (SYSTEM_CATEGORIES.stream().anyMatch(category -> category.id().equals(categoryId))) {
            throw EarthTripException.conflict(
                    "SYSTEM_CATEGORY_IMMUTABLE", "기본 지출 분류는 변경하거나 삭제할 수 없습니다.");
        }
    }
    ...
    private static String name(String value) {
        if (value == null || value.isBlank() || value.strip().length() > 100) {
            throw EarthTripException.badRequest(
                    "INVALID_EXPENSE_CATEGORY_NAME", "분류 이름은 1자 이상 100자 이하여야 합니다.");
        }
        return value.strip();
    }
    ...
    private static String color(String value) {
        ...
        if (!normalized.matches("#[0-9A-F]{6}")) {
            throw EarthTripException.badRequest(
                    "INVALID_EXPENSE_CATEGORY_COLOR", "분류 색상은 #RRGGBB 형식이어야 합니다.");
        }
        return normalized;
    }
    ...
    private static int order(int value) {
        if (value < 0) {
            throw EarthTripException.badRequest(
                    "INVALID_EXPENSE_CATEGORY_ORDER", "분류 정렬 순서는 0 이상이어야 합니다.");
        }
        return value;
    }

    private static void requireVersion(
            ExpenseCategoryStorePort.CategoryRecord category, long baseVersion) {
    ...
    private static EarthTripException idempotencyConflict() {
        return EarthTripException.conflict("IDEMPOTENCY_KEY_REUSED", "이미 다른 지출 분류에 사용된 요청 ID입니다.");
    }

    private record SystemCategory(UUID id, String code, String name, String color, int sortOrder) {}
}
