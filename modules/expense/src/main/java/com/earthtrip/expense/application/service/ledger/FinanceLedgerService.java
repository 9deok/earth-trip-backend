...
class FinanceLedgerService implements FinanceLedgerUseCase {
    ...
    private static final Set<String> MOVEMENT_TYPES =
            Set.of("EXCHANGE_IN", "EXCHANGE_OUT", "WITHDRAWAL", "INCOME", "EXPENSE", "ADJUSTMENT");
    private static final Set<String> NEGATIVE_MOVEMENT_TYPES = Set.of("EXCHANGE_OUT", "EXPENSE");
    ...
    public CashResult createCash(UUID tripId, UUID actorUserId, CashCommand command) {
        ...
        FinanceLedgerStorePort.CashRecord existing =
                store.cashById(command.requestId()).orElse(null);
        if (existing != null) {
        ...
        return cashResult(
                store.saveCash(
                        new FinanceLedgerStorePort.CashRecord(
    ...
    public CashResult updateCash(
            UUID tripId, UUID movementId, UUID actorUserId, CashCommand command) {
        access.requireEditor(tripId, actorUserId);
        FinanceLedgerStorePort.CashRecord current = loadCash(tripId, movementId);
        requireVersion(current, command.baseVersion());
        String nextType =
                command.movementType() == null
                        ? current.movementType()
                        : movementType(command.movementType());
        long sourceAmount =
                command.amountMinor() == 0 ? current.amountMinor() : command.amountMinor();
        FinanceLedgerStorePort.CashRecord updated =
                new FinanceLedgerStorePort.CashRecord(
                        movementId,
                        tripId,
                        nextType,
    ...
    @Override
    public void deleteCash(UUID tripId, UUID movementId, UUID actorUserId, long baseVersion) {
        access.requireEditor(tripId, actorUserId);
        FinanceLedgerStorePort.CashRecord current = loadCash(tripId, movementId);
    ...
    public CashResult reconcile(
        ...
        long estimated =
                balances(tripId, actorUserId).stream()
        ...
        try {
            ...
            throw EarthTripException.badRequest(
                    "CASH_BALANCE_OVERFLOW", "현금 잔액 차이가 지원 범위를 벗어났습니다.");
        }
        return createCash(
                tripId,
                actorUserId,
    ...
    public RateResult createRate(UUID tripId, UUID actorUserId, RateCommand command) {
        ...
        if (command.rate() == null || command.rate().signum() <= 0) {
            throw EarthTripException.badRequest("INVALID_EXCHANGE_RATE", "환율은 0보다 커야 합니다.");
        }
        FinanceLedgerStorePort.RateRecord existing =
                store.rateById(command.requestId()).orElse(null);
        if (existing != null) {
            if (!existing.tripId().equals(tripId)) {
                throw idempotencyConflict();
            }
        ...
        if (baseCurrency.equals(quoteCurrency)) {
            throw EarthTripException.badRequest("SAME_EXCHANGE_CURRENCY", "기준 통화와 상대 통화는 달라야 합니다.");
        }
        Instant now = clock.instant();
        return rateResult(
                store.saveRate(
                        new FinanceLedgerStorePort.RateRecord(
                                Objects.requireNonNull(command.requestId(), "요청 ID는 필수입니다."),
                                tripId,
                                baseCurrency,
                                quoteCurrency,
                                ...
                                command.source() == null
                                        ? "MANUAL"
                                        : command.source().strip().toUpperCase(Locale.ROOT),
                ...
                .orElseThrow(
                        () ->
                                EarthTripException.notFound(
                                        "CASH_MOVEMENT_NOT_FOUND", "현금 원장 항목을 찾을 수 없습니다."));
    }

    private static String movementType(String value) {
        if (value == null) {
            throw EarthTripException.badRequest("CASH_MOVEMENT_TYPE_REQUIRED", "현금 원장 유형이 필요합니다.");
        }
        String normalized = value.strip().toUpperCase(Locale.ROOT);
        if (!MOVEMENT_TYPES.contains(normalized)) {
            throw EarthTripException.badRequest(
                    "INVALID_CASH_MOVEMENT_TYPE", "지원하지 않는 현금 원장 유형입니다.");
        }
        return normalized;
    }
    ...
    private static long signedAmount(long value, String movementType) {
        if (value == 0 && !movementType.equals("ADJUSTMENT")) {
            throw EarthTripException.badRequest("INVALID_CASH_AMOUNT", "현금 원장 금액은 0일 수 없습니다.");
        }
        if (movementType.equals("ADJUSTMENT")) {
            return value;
        }
        if (value == Long.MIN_VALUE) {
            throw EarthTripException.badRequest("INVALID_CASH_AMOUNT", "현금 원장 금액이 지원 범위를 벗어났습니다.");
        }
    ...
    private static String status(String value) {
        ...
        if (!STATUSES.contains(normalized)) {
            throw EarthTripException.badRequest("INVALID_CASH_STATUS", "지원하지 않는 현금 원장 상태입니다.");
        }
        return normalized;
    }
    ...
    private static Instant requireOccurredAt(Instant occurredAt) {
        if (occurredAt == null) {
            throw EarthTripException.badRequest("OCCURRED_AT_REQUIRED", "현금 변동 시각이 필요합니다.");
        }
        return occurredAt;
    }
    ...
    private static void requireVersion(FinanceLedgerStorePort.CashRecord record, long baseVersion) {
    ...
    private static EarthTripException idempotencyConflict() {
        return EarthTripException.conflict("IDEMPOTENCY_KEY_REUSED", "이미 다른 작업에 사용된 요청 ID입니다.");
    }
}
