...
class SettlementPersistenceAdapter implements SettlementStorePort {
    ...
    public SettlementRecord save(SettlementRecord record) {
        ...
        SettlementJpaEntity entity = settlements.findById(record.id().toString())
            .map(existing -> {
    ...
    public PaymentRecord savePayment(PaymentRecord record) {
        SettlementPaymentJpaEntity entity = payments.findById(record.id().toString())
            .map(existing -> {
    ...
    public Optional<SupplementRecord> supplement(UUID supplementId) {
        return supplements.findById(supplementId.toString())
    ...
    private SettlementRecord record(SettlementJpaEntity entity) {
        try {
            return entity.record(json.readValue(
                entity.snapshot(),
                SettlementUseCase.PreviewResult.class
            ));
