...
class ExpenseService implements ExpenseUseCase {
    ...
    public ExpenseResult create(UUID tripId, UUID actorUserId, ExpenseCommand command) {
        ...
        Expense expense = Expense.create(
            command.requestId(), tripId, command.title(), category,
            required(command.amountMinor()), command.currency(), command.occurredAt(),
            command.payerContributions(), command.participantShares(),
    ...
    public ExpenseResult update(
        ...
        String category = command.categoryCode() == null
            ? null
            : categories.requireCategoryCode(tripId, command.categoryCode());
        expense.update(
            command.title(), category, command.amountMinor(), command.currency(),
    ...
    public List<ExpenseResult> split(
        ...
        if (!original.status().equals("RECORDED")) {
            throw EarthTripException.conflict(
                "EXPENSE_NOT_SPLITTABLE",
                "기록 상태의 지출만 분할할 수 있습니다."
            );
        }
        ...
        for (SplitPart part : parts) {
            if (part.requestId() == null || store.findById(part.requestId()).isPresent()) {
                throw EarthTripException.conflict(
                    ...
                    "분할 요청 ID가 이미 사용되었거나 비어 있습니다."
                );
            }
            String category = categories.requireCategoryCode(tripId, part.categoryCode());
            Expense child = Expense.create(
                part.requestId(), tripId, part.title(), category, part.amountMinor(),
                original.currency(), original.occurredAt(), part.payerContributions(),
                part.participantShares(), original.visibility(), original.note(), actorUserId, now
            );
    ...
    public AdjustmentResult refund(
        ...
        if (existing != null) {
            if (!existing.tripId().equals(tripId) || !existing.expenseId().equals(expenseId)
                || !existing.kind().equals("REFUND") || existing.amountMinor() != amountMinor
        ...
        long refunded;
        try {
            refunded = store.findAdjustments(expenseId).stream()
        ...
        Instant now = clock.instant();
        ExpenseStorePort.AdjustmentRecord saved = store.saveAdjustment(
    ...
    private static void validateSplit(Expense original, List<SplitPart> parts) {
        ...
        try {
            ...
            throw EarthTripException.badRequest(
                "INVALID_SPLIT_TOTAL",
                "분할 금액 합계가 지원 범위를 벗어났습니다."
            );
        }
        if (total != original.amountMinor()) {
            throw EarthTripException.badRequest(
                "INVALID_SPLIT_TOTAL",
                "분할 금액 합계가 원본 지출과 일치해야 합니다."
            );
        }
        if (parts.stream().map(SplitPart::requestId).distinct().count() != parts.size()) {
            throw EarthTripException.badRequest(
                "DUPLICATE_SPLIT_REQUEST_ID",
                "분할 항목의 요청 ID는 서로 달라야 합니다."
            );
        }
    }
    ...
    private static boolean visible(Expense expense, UUID actorUserId) {
        return switch (expense.visibility()) {
            ...
            case "PARTICIPANTS" -> expense.payers().containsKey(actorUserId)
    ...
    private static ExpenseResult result(Expense expense) {
        return new ExpenseResult(
            expense.id(), expense.tripId(), expense.title(), expense.categoryCode(),
            expense.amountMinor(), expense.currency(), expense.occurredAt(), expense.payers(),
            expense.shares(), expense.visibility(), expense.status(), expense.note(),
            expense.version(), expense.createdBy(), expense.updatedBy(), expense.createdAt(),
            expense.updatedAt()
        );
    }
    ...
    private static AdjustmentResult adjustment(ExpenseStorePort.AdjustmentRecord saved) {
        return new AdjustmentResult(
            saved.id(), saved.expenseId(), saved.kind(), saved.amountMinor(), saved.currency(),
            saved.participantId(), saved.payload(), saved.createdAt()
        );
    }
