...
class StatementImportService implements StatementImportUseCase {
    ...
    public ImportResult create(UUID tripId, UUID actorUserId, ImportCommand command) {
        ...
        StatementImportStorePort.ImportRecord existing = store
            .findImport(command.requestId())
            .orElse(null);
        if (existing != null) {
        ...
        StatementImportStorePort.ImportRecord saved = store.saveImport(
            new StatementImportStorePort.ImportRecord(
                command.requestId(), tripId, source(command.source()), "READY",
                actorUserId, now, now, 0
            )
        );
        for (CandidateCommand candidate : candidates) {
            validateCandidate(candidate.title(), candidate.amountMinor(), candidate.currency(),
                candidate.occurredAt(), candidate.payerUserId(), candidate.payload());
            StatementImportStorePort.CandidateRecord duplicate = store
            ...
            store.saveCandidate(new StatementImportStorePort.CandidateRecord(
                candidate.candidateId(), saved.id(), tripId, candidate.title().strip(),
                candidate.amountMinor(), currency(candidate.currency()), candidate.occurredAt(),
                candidate.payerUserId(), payload(candidate.payload()), "READY", null,
                now, now, 0
            ));
        }
        return result(saved);
    ...
    public CandidateResult updateCandidate(
        ...
        access.requireEditor(tripId, actorUserId);
        StatementImportStorePort.CandidateRecord current = loadCandidate(
            tripId, importId, candidateId
        );
        mutable(current);
        version(current.version(), command.baseVersion());
        String title = command.title() == null ? current.title() : command.title().strip();
        long amount = command.amountMinor() == null
            ? current.amountMinor()
            : command.amountMinor();
        ...
        Instant occurredAt = command.occurredAt() == null
            ? current.occurredAt()
            : command.occurredAt();
        UUID payer = command.payerUserId() == null
            ? current.payerUserId()
            : command.payerUserId();
        ...
        return result(store.saveCandidate(copy(
            current, title, amount, currency, occurredAt, payer, payload,
            current.status(), current.expenseId()
        )));
    }
    ...
    public CandidateResult linkExpense(
        ...
        if (!expense.currency().equals(current.currency())
            ...
            throw EarthTripException.badRequest(
                "STATEMENT_EXPENSE_MISMATCH",
                "통화와 금액이 같은 지출에만 카드 내역 후보를 연결할 수 있습니다."
            );
        }
        return result(store.saveCandidate(copy(
            current, current.title(), current.amountMinor(), current.currency(),
            current.occurredAt(), current.payerUserId(), current.payload(), "LINKED", expenseId
        )));
    }
    ...
    public CandidateResult unlinkExpense(
        ...
        return result(store.saveCandidate(copy(
            current, current.title(), current.amountMinor(), current.currency(),
            current.occurredAt(), current.payerUserId(), current.payload(), "READY", null
        )));
    }
    ...
    public CandidateResult dismiss(
        ...
        CandidateResult result = result(store.saveCandidate(copy(
            current, current.title(), current.amountMinor(), current.currency(),
            current.occurredAt(), current.payerUserId(), Map.copyOf(payload),
            "DISMISSED", null
        )));
    ...
    public ConfirmationResult confirm(
        ...
        for (ConfirmationItem item : items) {
            StatementImportStorePort.CandidateRecord candidate = loadCandidate(
                tripId, importId, item.candidateId()
            );
            ...
            ExpenseUseCase.ExpenseResult expense = expenses.create(
                tripId, actorUserId, new ExpenseUseCase.ExpenseCommand(
                    item.expenseRequestId(), candidate.title(), item.categoryCode(),
                    candidate.amountMinor(), candidate.currency(), candidate.occurredAt(),
                    Map.of(candidate.payerUserId(), candidate.amountMinor()),
                    item.participantShares(), item.visibility(), "RECORDED",
                    "카드 내역에서 가져옴", 0
                )
            );
            store.saveCandidate(copy(
                candidate, candidate.title(), candidate.amountMinor(), candidate.currency(),
                candidate.occurredAt(), candidate.payerUserId(), candidate.payload(),
                "CONFIRMED", expense.expenseId()
            ));
            created.add(expense);
        }
    ...
    private StatementImportStorePort.ImportRecord refreshImportStatus(
        UUID tripId,
        UUID importId
    ) {
        ...
        boolean complete = candidates.stream().allMatch(candidate ->
            Set.of("CONFIRMED", "LINKED", "DISMISSED").contains(candidate.status())
        );
        ...
        return store.saveImport(new StatementImportStorePort.ImportRecord(
            current.id(), tripId, current.source(), status, current.createdBy(),
    ...
    private StatementImportStorePort.CandidateRecord copy(
        ...
        return new StatementImportStorePort.CandidateRecord(
            current.id(), current.importId(), current.tripId(), title, amount, currency,
            occurredAt, payer, payload, status, expenseId, current.createdAt(),
    ...
    private ImportResult result(StatementImportStorePort.ImportRecord record) {
        return new ImportResult(
            record.id(), record.source(), record.status(),
            store.findCandidates(record.id()).size(), record.createdBy(), record.createdAt(),
    ...
    private static CandidateResult result(StatementImportStorePort.CandidateRecord record) {
        return new CandidateResult(
            record.id(), record.importId(), record.title(), record.amountMinor(),
            record.currency(), record.occurredAt(), record.payerUserId(), record.payload(),
            record.status(), record.expenseId(), record.createdAt(), record.updatedAt(),
            record.version()
        );
    }
    ...
    private static void validateCandidate(
        ...
        if (title == null || title.isBlank() || title.strip().length() > 200
            || amount <= 0 || occurredAt == null || payer == null
    ...
    private static String source(String value) {
        if (value == null || value.isBlank() || value.strip().length() > 80) {
            throw EarthTripException.badRequest(
                "INVALID_STATEMENT_SOURCE", "카드 내역 출처를 확인해 주세요."
            );
        }
    ...
    private static void version(long serverVersion, long baseVersion) {
        if (serverVersion != baseVersion) {
            throw new EarthTripException(
                "VERSION_CONFLICT", 409, "다른 카드 내역 변경이 먼저 저장되었습니다.",
