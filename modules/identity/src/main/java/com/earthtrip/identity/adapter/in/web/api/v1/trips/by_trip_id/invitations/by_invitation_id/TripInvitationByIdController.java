...
@RestController @RequestMapping("/api/v1/trips/{tripId}/invitations/{invitationId}")
class TripInvitationByIdController {
    ...
    @GetMapping InvitationResponse get(
    ...
    @PatchMapping InvitationResponse patch(
        @PathVariable UUID tripId, @PathVariable UUID invitationId,
        @Valid @RequestBody UpdateInvitationRequest request
    ) {
        InvitationUseCase.InvitationResult i = useCase.update(
            tripId, invitationId, currentUser.requireUserId(), request.role(),
            request.expiresAt(), request.baseVersion()
        );
        return response(i);
    }
