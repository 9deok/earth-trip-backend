...
class AccountIdentityPersistenceAdapter implements AccountIdentityStorePort {
    ...
    @Override
    public List<IdentityRecord> findByUser(UUID userId) {
    ...
    @Override
    public Optional<IdentityRecord> findIdentity(UUID id) {
        return identities.findById(id.toString()).map(LinkedIdentityJpaEntity::toRecord);
    }

    @Override
    public Optional<IdentityRecord> findIdentity(String provider, String subject) {
        return identities
    ...
    @Override
    public IdentityRecord saveIdentity(IdentityRecord record) {
        LinkedIdentityJpaEntity entity =
                identities
    ...
    public void deleteIdentity(UUID id) {
        identities.deleteById(id.toString());
    }

    @Override
    public Optional<EmailChangeRecord> findEmailChangeByTokenHash(String hash) {
        return emailChanges.findByTokenHash(hash).map(EmailChangeJpaEntity::toRecord);
    }

    @Override
    public Optional<EmailChangeRecord> findPendingEmailChange(UUID userId) {
        return emailChanges
                .findFirstByUserIdAndStatusOrderByCreatedAtDesc(userId.toString(), "PENDING")
                .map(EmailChangeJpaEntity::toRecord);
    }

    @Override
    public EmailChangeRecord saveEmailChange(EmailChangeRecord record) {
        EmailChangeJpaEntity entity =
                emailChanges
                        .findById(record.id().toString())
                        .map(
                                existing -> {
