...
class FavoriteCompanionService implements FavoriteCompanionUseCase {
    ...
    FavoriteCompanionService(
            PersonalSupportStorePort store, UserAccountStorePort users, Clock clock) {
        this.store = store;
        this.users = users;
        this.clock = clock;
    }
    ...
    public List<FavoriteResult> list(UUID actorUserId) {
        return store.favorites(actorUserId).stream().map(FavoriteCompanionService::result).toList();
    }
    ...
    public FavoriteResult add(
        ...
        ResolvedCompanion companion =
                resolveCompanion(actorUserId, companionUserId, displayName, email);
        PersonalSupportStorePort.FavoriteRecord duplicate =
        ...
        return result(
                store.saveFavorite(
                        new PersonalSupportStorePort.FavoriteRecord(
                                requestId,
                                actorUserId,
                                companion.userId(),
    ...
    public void remove(UUID actorUserId, UUID favoriteId) {
        PersonalSupportStorePort.FavoriteRecord favorite =
                store.favorite(favoriteId)
    ...
    private ResolvedCompanion resolveCompanion(
            UUID actorUserId, UUID companionUserId, String displayName, String email) {
        if (companionUserId != null) {
            if (companionUserId.equals(actorUserId)) {
                throw EarthTripException.badRequest(
                        "CANNOT_FAVORITE_SELF", "자기 자신은 동행자 즐겨찾기에 추가할 수 없습니다.");
            }
            UserAccount account =
                    users.findById(new UserId(companionUserId))
        ...
        try {
            ...
            throw EarthTripException.badRequest("INVALID_COMPANION_EMAIL", "동행자 이메일을 확인해 주세요.");
        }
    ...
    private static String displayName(String value) {
        if (value == null || value.isBlank() || value.strip().length() > 80) {
            throw EarthTripException.badRequest(
                    "INVALID_COMPANION_NAME", "동행자 표시 이름은 1자 이상 80자 이하여야 합니다.");
        }
        return value.strip();
    }
    ...
    private static EarthTripException idempotencyConflict() {
        return EarthTripException.conflict("IDEMPOTENCY_KEY_REUSED", "이미 다른 즐겨찾기에 사용된 요청 ID입니다.");
    }
