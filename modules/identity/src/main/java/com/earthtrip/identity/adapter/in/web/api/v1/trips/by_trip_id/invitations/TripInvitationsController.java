...
@RestController @RequestMapping("/api/v1/trips/{tripId}/invitations")
class TripInvitationsController {
    ...
    @GetMapping List<InvitationResponse> get(@PathVariable UUID tripId) {
        return useCase.list(tripId, currentUser.requireUserId()).stream()
            .map(TripInvitationsController::response).toList();
    }
    @PostMapping @ResponseStatus(HttpStatus.CREATED)
    CreatedInvitationResponse post(
        @PathVariable UUID tripId, @Valid @RequestBody CreateInvitationRequest request
    ) {
        InvitationUseCase.CreatedInvitation created = useCase.create(
            tripId, currentUser.requireUserId(), request.requestId(), request.email(), request.role()
        );
    ...
    private static InvitationResponse response(InvitationUseCase.InvitationResult i) {
        return new InvitationResponse(
            i.invitationId(), i.tripId(), i.email(), i.role(), i.status(), i.expiresAt(),
            i.deliveryStatus(), i.lastDeliveredAt(), i.createdAt(), i.version()
        );
    }
}
record CreateInvitationRequest(@NotNull UUID requestId, @NotBlank @Email String email, String role) { }
record CreatedInvitationResponse(InvitationResponse invitation, String token, String invitationUrl) { }
record InvitationResponse(
    UUID invitationId, UUID tripId, String email, String role, String status, Instant expiresAt,
    String deliveryStatus, Instant lastDeliveredAt, Instant createdAt, long version
) { }
