...
class DataExportDownloadService implements DataExportDownloadUseCase {
    ...
    public DownloadResult download(UUID actorUserId, UUID exportId) {
        DataExportStorePort.ExportRecord record = store.findById(exportId)
        ...
        if (record.expiresAt() == null || !record.expiresAt().isAfter(clock.instant())) {
            throw EarthTripException.notFound(
                "DATA_EXPORT_EXPIRED",
                "개인정보 내보내기 다운로드 기간이 만료되었습니다."
            );
        }
        PersonalDataExporterPort.DownloadArtifact artifact = exporter.download(
            actorUserId,
            exportId,
            record.format()
        );
