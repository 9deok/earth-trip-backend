...
@Entity @Table(name = "email_change_requests")
class EmailChangeJpaEntity {
    ...
    @Column(name = "new_email", nullable = false, length = 320) private String newEmail;
    ...
    EmailChangeJpaEntity(AccountIdentityStorePort.EmailChangeRecord record) {
        id = record.id().toString(); userId = record.userId().toString();
        newEmail = record.newEmail(); tokenHash = record.tokenHash(); createdAt = record.createdAt();
        apply(record);
    }
    void apply(AccountIdentityStorePort.EmailChangeRecord record) {
        status = record.status(); expiresAt = record.expiresAt(); confirmedAt = record.confirmedAt();
    }
    AccountIdentityStorePort.EmailChangeRecord toRecord() {
        return new AccountIdentityStorePort.EmailChangeRecord(
            UUID.fromString(id), UUID.fromString(userId), newEmail, tokenHash, status,
