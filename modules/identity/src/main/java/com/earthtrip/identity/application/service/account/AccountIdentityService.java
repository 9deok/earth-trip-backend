...
class AccountIdentityService implements AccountIdentityUseCase {
    ...
    public List<IdentityResult> list(UUID actorUserId) {
        ...
        return identities.findByUser(actorUserId).stream()
            .map(AccountIdentityService::result).toList();
    }
    ...
    public IdentityResult link(UUID actorUserId, String provider, OAuthCommand command) {
        ...
        AccountIdentityStorePort.IdentityRecord existing = identities
            .findIdentity(normalized, verified.subject()).orElse(null);
        if (existing != null) {
        ...
        return result(identities.saveIdentity(new AccountIdentityStorePort.IdentityRecord(
            UUID.randomUUID(), actorUserId, normalized, verified.subject(), verified.email(),
            now, now, 0
        )));
    }
    ...
    public void unlink(UUID actorUserId, UUID identityId) {
        loadAccount(actorUserId);
        AccountIdentityStorePort.IdentityRecord identity = identities.findIdentity(identityId)
            .filter(item -> item.userId().equals(actorUserId))
            .orElseThrow(() -> EarthTripException.notFound(
                "LINKED_IDENTITY_NOT_FOUND", "연결된 로그인 수단을 찾을 수 없습니다."
            ));
        if (identities.findByUser(actorUserId).size() <= 1) {
            throw EarthTripException.conflict(
                "LAST_LOGIN_IDENTITY", "마지막 외부 로그인 수단은 해제할 수 없습니다."
            );
        }
    ...
    public SessionUseCase.SessionResult oauthSession(String provider, OAuthCommand command) {
        String normalized = provider(provider);
        OAuthProviderPort.VerifiedIdentity verified = verify(normalized, command);
        AccountIdentityStorePort.IdentityRecord identity = identities
            .findIdentity(normalized, verified.subject()).orElse(null);
        UserAccount account;
        if (identity == null) {
            account = createOAuthAccount(verified);
            Instant now = clock.instant();
            identity = identities.saveIdentity(new AccountIdentityStorePort.IdentityRecord(
                UUID.randomUUID(), account.id().value(), normalized, verified.subject(),
                verified.email(), now, now, 0
            ));
        } else {
    ...
    public EmailChangeRequestResult requestEmailChange(UUID actorUserId, String rawEmail) {
        ...
        if (newEmail.equals(account.email())) {
            throw EarthTripException.badRequest(
                "EMAIL_UNCHANGED", "현재 이메일과 다른 이메일을 입력해 주세요."
            );
        }
        ...
        Instant now = clock.instant();
        identities.findPendingEmailChange(actorUserId).ifPresent(previous ->
            identities.saveEmailChange(new AccountIdentityStorePort.EmailChangeRecord(
                previous.id(), previous.userId(), previous.newEmail(), previous.tokenHash(),
                "SUPERSEDED", previous.expiresAt(), previous.createdAt(), null
            ))
        );
        String rawToken = credentials.newToken();
        Instant expiresAt = now.plus(EMAIL_CHANGE_TTL);
        AccountIdentityStorePort.EmailChangeRecord saved = identities.saveEmailChange(
            new AccountIdentityStorePort.EmailChangeRecord(
                UUID.randomUUID(), actorUserId, newEmail.value(),
                credentials.hashToken(rawToken), "PENDING", expiresAt, now, null
            )
        );
        VerificationDeliveryPort.DeliveryStatus status = delivery.sendEmailChange(
            newEmail, rawToken, expiresAt
        );
    ...
    public EmailChangeResult confirmEmailChange(UUID actorUserId, String rawToken) {
        if (rawToken == null || rawToken.isBlank()) {
            throw EarthTripException.badRequest(
                "EMAIL_CHANGE_TOKEN_REQUIRED", "이메일 변경 토큰이 필요합니다."
            );
        }
        AccountIdentityStorePort.EmailChangeRecord request = identities
            .findEmailChangeByTokenHash(credentials.hashToken(rawToken))
            .filter(item -> item.userId().equals(actorUserId))
            .orElseThrow(() -> EarthTripException.badRequest(
                "INVALID_EMAIL_CHANGE_TOKEN", "만료되었거나 올바르지 않은 이메일 변경 토큰입니다."
            ));
        Instant now = clock.instant();
        ...
        accounts.findByEmail(newEmail).filter(other -> !other.id().value().equals(actorUserId))
            .ifPresent(other -> {
                throw EarthTripException.conflict(
                    "EMAIL_ALREADY_REGISTERED", "이미 가입된 이메일입니다."
                );
        ...
        identities.saveEmailChange(new AccountIdentityStorePort.EmailChangeRecord(
            request.id(), request.userId(), request.newEmail(), request.tokenHash(),
            "CONFIRMED", request.expiresAt(), request.createdAt(), now
        ));
    ...
    private OAuthProviderPort.VerifiedIdentity verify(String provider, OAuthCommand command) {
        ...
        return oauth.verify(provider, new OAuthProviderPort.OAuthCredential(
            command.authorizationCode(), command.idToken(), command.redirectUri(),
    ...
    private UserAccount createOAuthAccount(OAuthProviderPort.VerifiedIdentity verified) {
        ...
        Instant now = clock.instant();
        String displayName = verified.displayName() == null || verified.displayName().isBlank()
            ? email.value().substring(0, email.value().indexOf('@'))
            : verified.displayName();
        UserAccount account = UserAccount.register(
            new UserId(UUID.randomUUID()), email,
            credentials.hashPassword(credentials.newToken()), displayName, now
        );
    ...
    private SessionUseCase.SessionResult issueSession(UserAccount account, String rawDeviceName) {
        ...
        AuthSession session = AuthSession.create(
            UUID.randomUUID(), account.id(), credentials.hashToken(accessToken),
            credentials.hashToken(refreshToken),
            rawDeviceName == null || rawDeviceName.isBlank() ? "OAuth device" : rawDeviceName,
            now.plus(ACCESS_TTL), now.plus(REFRESH_TTL), now
        );
        sessions.save(session);
        return new SessionUseCase.SessionResult(
            session.id(), account.id().value(), accessToken, refreshToken,
    ...
    private AccountIdentityStorePort.IdentityRecord touch(
        ...
        return identities.saveIdentity(new AccountIdentityStorePort.IdentityRecord(
            identity.id(), identity.userId(), identity.provider(), identity.providerSubject(),
            email, identity.createdAt(), clock.instant(), identity.version()
        ));
    }

    private UserAccount loadAccount(UUID userId) {
        return accounts.findById(new UserId(userId)).orElseThrow(() -> EarthTripException.notFound(
    ...
    private static IdentityResult result(AccountIdentityStorePort.IdentityRecord identity) {
        return new IdentityResult(
            identity.id(), identity.provider(), identity.providerEmail(), identity.createdAt(),
            identity.lastUsedAt(), identity.version()
        );
    }
}
