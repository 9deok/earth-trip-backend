...
class TripMemberPersistenceAdapter implements TripMemberStorePort, TripMembershipLookup {
    ...
    TripMemberPersistenceAdapter(TripMemberJpaRepository repository) {
        this.repository = repository;
    }

    @Override
    public List<MemberRecord> findAll(UUID tripId) {
    ...
    @Override
    public Optional<MemberRecord> findByTripAndUser(UUID tripId, UUID userId) {
        return repository
                .findByTripIdAndUserId(tripId.toString(), userId.toString())
                .map(TripMemberJpaEntity::toRecord);
    }

    @Override
    public MemberRecord save(MemberRecord member) {
        TripMemberJpaEntity entity =
                repository
                        .findById(member.id().toString())
                        .map(
                                existing -> {
    ...
    public void delete(UUID memberId) {
        repository.deleteById(memberId.toString());
    }

    @Override
    public Optional<String> activeRole(UUID tripId, UUID userId) {
        return findByTripAndUser(tripId, userId)
    ...
    public List<UUID> activeTripIds(UUID userId) {
        return repository.findAllByUserIdAndStatus(userId.toString(), "ACTIVE").stream()
                .map(entity -> entity.toRecord().tripId())
                .distinct()
                .toList();
    }
}
