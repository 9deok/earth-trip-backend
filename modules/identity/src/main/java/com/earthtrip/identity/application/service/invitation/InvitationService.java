...
class InvitationService implements InvitationUseCase {
    ...
    public CreatedInvitation create(
        ...
        EmailAddress email = new EmailAddress(rawEmail); String role = role(rawRole);
        InvitationStorePort.InvitationRecord byId = invitations.findById(requestId).orElse(null);
        if (byId != null) {
            throw EarthTripException.conflict(
                "IDEMPOTENCY_RESULT_REDACTED",
                "초대 토큰은 최초 응답에서만 제공됩니다. 새 전송 요청을 사용해 주세요."
            );
        }
        invitations.findActive(tripId, email.value()).ifPresent(existing -> {
            throw EarthTripException.conflict("ACTIVE_INVITATION_EXISTS", "이미 진행 중인 초대가 있습니다.");
        });
        Instant now = clock.instant(); String rawToken = credentials.newToken();
        InvitationStorePort.InvitationRecord invitation = new InvitationStorePort.InvitationRecord(
            requestId, tripId, email.value(), role, credentials.hashToken(rawToken), "PENDING",
    ...
    public InvitationResult update(
        UUID tripId, UUID invitationId, UUID actorUserId, String rawRole, Instant expiresAt, long baseVersion
    ) {
        tripAccess.requireOwner(tripId, actorUserId);
        InvitationStorePort.InvitationRecord current = loadInTrip(tripId, invitationId);
        requirePending(current); verifyVersion(current, baseVersion);
    ...
    public void revoke(UUID tripId, UUID invitationId, UUID actorUserId, long baseVersion) {
        tripAccess.requireOwner(tripId, actorUserId);
        InvitationStorePort.InvitationRecord current = loadInTrip(tripId, invitationId);
        requirePending(current); verifyVersion(current, baseVersion); Instant now = clock.instant();
        invitations.save(copy(
            current, current.role(), current.tokenHash(), "REVOKED", current.invitedUserId(),
    ...
    public CreatedInvitation redeliver(
        ...
        InvitationStorePort.InvitationRecord current = loadInTrip(tripId, invitationId);
        requirePending(current); verifyVersion(current, baseVersion);
        Instant now = clock.instant(); String token = credentials.newToken();
        InvitationStorePort.InvitationRecord rotated = copy(
            current, current.role(), credentials.hashToken(token), "PENDING", current.invitedUserId(),
    ...
    public PreviewResult preview(String rawToken) {
        ...
        return new PreviewResult(
            invitation.id(), trip.tripId(), trip.title(), trip.startDate(), trip.endDate(),
            maskEmail(invitation.email()), invitation.role(), status, invitation.expiresAt()
        );
    }
    ...
    public void accept(String rawToken, UUID actorUserId) {
        InvitationStorePort.InvitationRecord invitation = loadToken(rawToken);
        requireUsable(invitation); UserAccount user = users.findById(new UserId(actorUserId))
            .orElseThrow(() -> EarthTripException.notFound("ACCOUNT_NOT_FOUND", "계정을 찾을 수 없습니다."));
        if (!user.email().value().equals(invitation.email())) throw EarthTripException.forbidden(
            "INVITATION_EMAIL_MISMATCH", "초대를 받은 이메일 계정으로 로그인해 주세요."
        );
        Instant now = clock.instant();
        members.findByTripAndUser(invitation.tripId(), actorUserId).orElseGet(() -> members.save(
            new TripMemberStorePort.MemberRecord(
                UUID.randomUUID(), invitation.tripId(), actorUserId, invitation.role(), "ACTIVE", now, now, 0
            )
        ));
        invitations.save(copy(
            invitation, invitation.role(), invitation.tokenHash(), "ACCEPTED", actorUserId,
    ...
    public void decline(String rawToken) {
        InvitationStorePort.InvitationRecord invitation = loadToken(rawToken); requireUsable(invitation);
        Instant now = clock.instant();
        invitations.save(copy(
            invitation, invitation.role(), invitation.tokenHash(), "DECLINED", null,
    ...
    private CreatedInvitation deliver(
        ...
        InvitationStorePort.InvitationRecord saved = invitations.save(copy(
            ...
            invitation.invitedUserId(), invitation.expiresAt(), invitation.acceptedAt(),
            invitation.declinedAt(), invitation.revokedAt(), now, status, now
        ));
    ...
    private InvitationStorePort.InvitationRecord loadToken(String token) {
        ...
        return invitations.findByTokenHash(credentials.hashToken(token)).orElseThrow(InvitationService::invalidToken);
    }
    private InvitationStorePort.InvitationRecord loadInTrip(UUID tripId, UUID id) {
        return invitations.findById(id).filter(i -> i.tripId().equals(tripId))
            .orElseThrow(() -> EarthTripException.notFound("INVITATION_NOT_FOUND", "초대를 찾을 수 없습니다."));
    }
    ...
    private static void requirePending(InvitationStorePort.InvitationRecord i) {
        if (!i.status().equals("PENDING")) throw EarthTripException.conflict(
            "INVITATION_NOT_PENDING", "대기 중인 초대만 변경할 수 있습니다."
        );
    }
    private static String effectiveStatus(InvitationStorePort.InvitationRecord i, Instant now) {
        return i.status().equals("PENDING") && !now.isBefore(i.expiresAt()) ? "EXPIRED" : i.status();
    }
    private static String role(String raw) {
        String value = raw == null ? "EDITOR" : raw.strip().toUpperCase(Locale.ROOT);
        if (!value.equals("EDITOR") && !value.equals("VIEWER")) throw EarthTripException.badRequest(
    ...
    private static void verifyVersion(InvitationStorePort.InvitationRecord i, long baseVersion) {
        if (i.version() != baseVersion) throw new EarthTripException(
            "VERSION_CONFLICT", 409, "다른 초대 변경이 먼저 저장되었습니다.",
    ...
    private static InvitationResult result(InvitationStorePort.InvitationRecord i) {
        return new InvitationResult(
            i.id(), i.tripId(), i.email(), i.role(), i.status(), i.expiresAt(), i.deliveryStatus(),
            i.lastDeliveredAt(), i.createdAt(), i.version()
        );
    }
    private static InvitationStorePort.InvitationRecord copy(
        InvitationStorePort.InvitationRecord i, String role, String tokenHash, String status,
        UUID invitedUserId, Instant expiresAt, Instant acceptedAt, Instant declinedAt,
        Instant revokedAt, Instant lastDeliveredAt, String deliveryStatus, Instant updatedAt
    ) {
        return new InvitationStorePort.InvitationRecord(
            i.id(), i.tripId(), i.email(), role, tokenHash, status, i.invitedBy(), invitedUserId,
            expiresAt, acceptedAt, declinedAt, revokedAt, lastDeliveredAt, deliveryStatus,
            i.createdAt(), updatedAt, i.version()
        );
    }
}
