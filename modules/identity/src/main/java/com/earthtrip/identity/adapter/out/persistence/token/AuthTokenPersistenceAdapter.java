...
class AuthTokenPersistenceAdapter implements AuthTokenStorePort {
    ...
    public AuthToken save(AuthToken token) {
        AuthTokenJpaEntity entity =
                repository
                        .findById(token.id().toString())
                        .map(
                                existing -> {
    ...
    public Optional<AuthToken> findUsableByHash(String tokenHash, AuthToken.Purpose purpose) {
        return repository
                .findByTokenHashAndPurposeAndConsumedAtIsNull(tokenHash, purpose.name())
    ...
    public void invalidateFor(UserId userId, AuthToken.Purpose purpose, Instant now) {
        repository
                .findAllByUserIdAndPurposeAndConsumedAtIsNull(userId.toString(), purpose.name())
                .forEach(
                        entity -> {
                            ...
                            try {
                                ...
                                // Expired tokens are already unusable and may remain for audit
                                // retention.
                            }
                            entity.apply(token);
                        });
