...
class InvitationJpaEntity {
    ...
    @Column(name = "invited_user_id", length = 36)
    ...
    InvitationJpaEntity(InvitationStorePort.InvitationRecord r) {
        id = r.id().toString();
        apply(r);
    }
    ...
    void apply(InvitationStorePort.InvitationRecord r) {
        ...
        status = r.status();
        invitedBy = r.invitedBy().toString();
    ...
    InvitationStorePort.InvitationRecord toRecord() {
        return new InvitationStorePort.InvitationRecord(
                ...
                tokenHash,
                status,
                UUID.fromString(invitedBy),
                invitedUserId == null ? null : UUID.fromString(invitedUserId),
                expiresAt,
                acceptedAt,
                declinedAt,
                revokedAt,
