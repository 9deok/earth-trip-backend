...
class PolicyService implements PolicyUseCase {
    ...
    @Override
    public ConsentResult decide(
            UUID userId, String policyId, String rawDecision, String rawSource) {
        PolicyStorePort.PolicyRecord policy =
                store.findActivePolicy(policyId)
                        .orElseThrow(
                                () ->
        ...
        if (policy.required() && !decision.equals("ACCEPTED")) {
            throw EarthTripException.badRequest(
                    "REQUIRED_POLICY_CANNOT_BE_DECLINED", "필수 정책은 동의 철회 대신 계정 탈퇴 절차를 사용해야 합니다.");
        }
        String source =
                rawSource == null || rawSource.isBlank()
                        ? "APP"
                        : rawSource.strip().toUpperCase(Locale.ROOT);
        return consentResult(store.saveConsent(userId, policy, decision, source, clock.instant()));
    }
    ...
    private static String normalizeDecision(String rawDecision) {
        ...
        if (!decision.equals("ACCEPTED") && !decision.equals("DECLINED")) {
            throw EarthTripException.badRequest(
                    "INVALID_POLICY_DECISION", "정책 결정은 ACCEPTED 또는 DECLINED여야 합니다.");
        }
        return decision;
    }
