...
class TripMemberService implements TripMemberUseCase {
    ...
    public List<MemberResult> list(UUID tripId, UUID actorUserId) {
        ...
        UserAccount owner = loadUser(access.ownerUserId());
        result.add(new MemberResult(
            syntheticOwnerMemberId(tripId, owner.id().value()), owner.id().value(), owner.displayName(),
            owner.email().value(), "OWNER", "ACTIVE", owner.id().value().equals(actorUserId),
            owner.createdAt(), access.tripVersion()
        ));
        members.findAll(tripId).stream().filter(member -> member.status().equals("ACTIVE"))
            .forEach(member -> {
                UserAccount user = loadUser(member.userId());
                result.add(new MemberResult(
                    member.id(), member.userId(), user.displayName(), user.email().value(), member.role(),
                    member.status(), member.userId().equals(actorUserId), member.joinedAt(), member.version()
                ));
            });
    ...
    public MemberResult changeRole(
        ...
        if (role.equals("OWNER")) throw EarthTripException.badRequest(
            "USE_OWNERSHIP_TRANSFER", "소유자 변경은 소유권 이전 절차를 사용해 주세요."
        );
        TripMemberStorePort.MemberRecord saved = members.save(new TripMemberStorePort.MemberRecord(
    ...
    public void leave(UUID tripId, UUID actorUserId) {
        TripAccess.AccessResult access = tripAccess.requireViewer(tripId, actorUserId);
        if (access.ownerUserId().equals(actorUserId)) throw EarthTripException.conflict(
    ...
    public void transferOwnership(
        ...
        tripAccess.requireOwner(tripId, actorUserId);
        if (!confirmed) throw EarthTripException.badRequest(
        ...
        if (!target.status().equals("ACTIVE")) throw EarthTripException.conflict(
        ...
        TripMemberStorePort.MemberRecord formerOwner = members.findByTripAndUser(tripId, actorUserId)
            .orElse(new TripMemberStorePort.MemberRecord(
                UUID.randomUUID(), tripId, actorUserId, "EDITOR", "ACTIVE", now, now, 0
            ));
        members.save(new TripMemberStorePort.MemberRecord(
            formerOwner.id(), tripId, actorUserId, "EDITOR", "ACTIVE",
    ...
    private TripMemberStorePort.MemberRecord loadMember(UUID tripId, UUID memberId) {
        return members.findById(memberId).filter(member -> member.tripId().equals(tripId))
            .orElseThrow(() -> EarthTripException.notFound("MEMBER_NOT_FOUND", "여행 멤버를 찾을 수 없습니다."));
    }
    ...
    private MemberResult result(TripMemberStorePort.MemberRecord member, UUID currentUserId) {
        ...
        return new MemberResult(
            member.id(), member.userId(), user.displayName(), user.email().value(), member.role(),
            member.status(), member.userId().equals(currentUserId), member.joinedAt(), member.version()
        );
    }
    ...
    private static void verifyVersion(TripMemberStorePort.MemberRecord member, long version) {
        if (member.version() != version) throw new EarthTripException(
            "VERSION_CONFLICT", 409, "다른 권한 변경이 먼저 저장되었습니다.",
            java.util.Map.of("serverVersion", member.version())
        );
    }
    private static UUID syntheticOwnerMemberId(UUID tripId, UUID ownerId) {
        return UUID.nameUUIDFromBytes((tripId + ":owner:" + ownerId).getBytes(StandardCharsets.UTF_8));
    }
}
