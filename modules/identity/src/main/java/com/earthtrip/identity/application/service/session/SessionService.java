...
class SessionService implements SessionUseCase {
    ...
    public SessionResult create(String email, String password, String deviceName) {
        UserAccount account =
                accountStore
                        .findByEmail(new EmailAddress(email))
        ...
        if (!account.canSignIn()) {
            String code =
                    account.status() == UserAccount.Status.PENDING_VERIFICATION
    ...
    public SessionResult refresh(String rawRefreshToken) {
        ...
        AuthSession session =
                sessionStore
                        .findByRefreshTokenHash(credentialPort.hashToken(rawRefreshToken))
                        .orElseThrow(
                                () ->
                                        EarthTripException.unauthorized(
                                                "INVALID_REFRESH_TOKEN", "올바르지 않은 갱신 토큰입니다."));
        Instant now = clock.instant();
    ...
    private static SessionResult result(
            AuthSession session, String accessToken, String refreshToken) {
    ...
    private static EarthTripException invalidCredentials() {
        return EarthTripException.unauthorized("INVALID_CREDENTIALS", "이메일 또는 비밀번호가 올바르지 않습니다.");
    }
}
