...
class SupportRequestService implements SupportRequestUseCase {
    ...
    private static final Set<String> CATEGORIES = Set.of(
        "ACCOUNT", "SYNC", "DATA", "PAYMENT", "ROUTE", "RESERVATION", "OTHER"
    );
    private static final Set<String> DIAGNOSTIC_KEYS = Set.of(
        "appVersion", "platform", "osVersion", "deviceModel", "lastRoute",
        "networkState", "providerStates", "syncCursor", "errorCode"
    );
    ...
    public SupportResult create(
        ...
        String diagnosticsJson = diagnostics(
            diagnostics, diagnosticsConsent
        );
        return result(store.saveSupport(new PersonalSupportStorePort.SupportRecord(
            requestId, actorUserId, safeCategory, safeDescription, safeTraceId,
            diagnosticsJson, "OPEN", clock.instant()
        )));
    }
    ...
    private String diagnostics(Map<String, Object> value, boolean consent) {
        ...
        if (!consent) {
            throw EarthTripException.badRequest(
                ...
                "진단정보를 첨부하려면 명시적으로 동의해야 합니다."
            );
        }
        if (!DIAGNOSTIC_KEYS.containsAll(value.keySet())) {
            throw EarthTripException.badRequest(
                "UNSUPPORTED_DIAGNOSTIC_FIELD",
                "허용되지 않은 진단정보 필드가 포함되어 있습니다."
            );
        }
        try {
            String json = objectMapper.writeValueAsString(new LinkedHashMap<>(value));
            if (json.length() > 10_000) {
                throw EarthTripException.badRequest(
                    "DIAGNOSTICS_TOO_LARGE",
                    "진단정보는 10000자 이하여야 합니다."
                );
            }
            return json;
    ...
    private static String description(String value) {
        if (value == null || value.isBlank() || value.strip().length() > 5_000) {
            throw EarthTripException.badRequest(
                ...
                "지원 요청 내용은 1자 이상 5000자 이하여야 합니다."
            );
        }
        return value.strip();
    }
    ...
    private static SupportResult result(PersonalSupportStorePort.SupportRecord record) {
        return new SupportResult(record.id(), record.status(), record.createdAt());
    }
}
