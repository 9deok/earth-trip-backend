...
@Entity @Table(name = "linked_identities")
class LinkedIdentityJpaEntity {
    ...
    LinkedIdentityJpaEntity(AccountIdentityStorePort.IdentityRecord record) {
        ...
        provider = record.provider(); providerSubject = record.providerSubject();
        createdAt = record.createdAt(); apply(record);
    }
    void apply(AccountIdentityStorePort.IdentityRecord record) {
        providerEmail = record.providerEmail(); lastUsedAt = record.lastUsedAt();
    }
    AccountIdentityStorePort.IdentityRecord toRecord() {
        return new AccountIdentityStorePort.IdentityRecord(
            UUID.fromString(id), UUID.fromString(userId), provider, providerSubject,
            providerEmail, createdAt, lastUsedAt, version
        );
    }
}
