...
class IdentitySecurityConfiguration {
    ...
    @Bean
    BearerTokenFilter bearerTokenFilter(AccessTokenAuthenticationUseCase authenticationUseCase) {
        return new BearerTokenFilter(authenticationUseCase);
    }

    @Bean
    InternalTokenFilter internalTokenFilter(org.springframework.core.env.Environment environment) {
        return new InternalTokenFilter(
                environment.getProperty("earthtrip.internal.admin-token", ""));
    }
                ...
                .authorizeHttpRequests(
                        authorize ->
                                authorize
                                        ...
                                        .permitAll()
                                        .requestMatchers("/internal/**", "/actuator/prometheus")
                                        .permitAll()
                                        .requestMatchers(HttpMethod.GET, "/api/v1/app-capabilities")
                                        .permitAll()
                                        .requestMatchers(
