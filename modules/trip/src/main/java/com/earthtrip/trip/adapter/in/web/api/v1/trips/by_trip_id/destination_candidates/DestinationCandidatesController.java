...
@RestController
@RequestMapping("/api/v1/trips/{tripId}/destination-candidates")
class DestinationCandidatesController {
    ...
    @GetMapping
    List<DestinationCandidateResponse> get(@PathVariable UUID tripId) {
        return useCase.list(tripId, actor.requireUserId()).stream()
                .map(DestinationCandidatesController::response)
                .toList();
    }
