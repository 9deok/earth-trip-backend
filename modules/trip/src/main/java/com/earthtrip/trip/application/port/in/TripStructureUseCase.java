...
public interface TripStructureUseCase {
    ...
    ChangeSetResult apply(
            UUID tripId, UUID actorUserId, StructureProposal proposal, String expectedProposalHash);

    ChangeSetResult synchronize(UUID tripId, UUID actorUserId, StructureProposal proposal);

    ChangeSetResult revert(UUID tripId, UUID changeSetId, UUID actorUserId, long baseVersion);

    List<DiagnosticResult> diagnostics(UUID tripId, UUID actorUserId);

    DiagnosticResult resolve(UUID tripId, UUID diagnosticId, UUID actorUserId, String note);
