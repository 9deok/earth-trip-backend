...
public final class DateCandidate {
    ...
    private DateCandidate(
        ...
        apply(startDate, endDate, note, status, updatedAt);
    ...
    public static DateCandidate restore(
        ...
        return new DateCandidate(
                id, tripId, start, end, note, status, actor, created, updated, version);
    }
    ...
    public void update(LocalDate start, LocalDate end, String note, String status, Instant now) {
        apply(
                ...
                status == null
                        ? this.status
                        : Status.valueOf(status.strip().toUpperCase(Locale.ROOT)),
                now);
    }
