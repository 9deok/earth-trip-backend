...
class DestinationCandidateService implements DestinationCandidateUseCase {
    ...
    private final DestinationCandidateStorePort store;
    private final TripChangePublisher changes;
    private final Clock clock;

    DestinationCandidateService(
            TripAccess access,
            DestinationCandidateStorePort store,
    ...
    public List<CandidateResult> list(UUID tripId, UUID actor) {
        ...
        List<DestinationCandidate> rows = store.findAll(new TripId(tripId));
        Map<UUID, List<DestinationCandidateStorePort.PreferenceRecord>> preferences =
                store.preferences(rows.stream().map(DestinationCandidate::id).toList());
        return rows.stream()
                .map(row -> result(row, preferences.getOrDefault(row.id(), List.of())))
                .toList();
    }

    @Override
    public CandidateResult create(UUID tripId, UUID actor, CandidateCommand c) {
    ...
    @Override
    public CandidateResult update(UUID tripId, UUID id, UUID actor, CandidateCommand c) {
        access.requireEditor(tripId, actor);
        DestinationCandidate candidate = load(tripId, id);
    ...
    @Override
    public void delete(UUID tripId, UUID id, UUID actor, long baseVersion) {
        access.requireEditor(tripId, actor);
        DestinationCandidate candidate = load(tripId, id);
        verifyVersion(candidate, baseVersion);
        store.delete(id);
        changes.publish(tripId, actor, "DELETED", "DESTINATION_CANDIDATE", id);
    }

    @Override
    public PreferenceResult putPreference(UUID tripId, UUID id, UUID actor, String raw) {
        access.requireViewer(tripId, actor);
        load(tripId, id);
        ...
        PreferenceResult result =
                preference(store.savePreference(id, actor, preference, clock.instant()));
        changes.publish(tripId, actor, "VOTED", "DESTINATION_CANDIDATE", id);
        return result;
    }

    @Override
    public void deletePreference(UUID tripId, UUID id, UUID actor) {
        access.requireViewer(tripId, actor);
        load(tripId, id);
        store.deletePreference(id, actor);
    ...
    private DestinationCandidate load(UUID tripId, UUID id) {
        return store.findById(id)
                .filter(c -> c.tripId().value().equals(tripId))
                .orElseThrow(
                        () ->
                                EarthTripException.notFound(
                                        "DESTINATION_CANDIDATE_NOT_FOUND", "여행지 후보를 찾을 수 없습니다."));
    }
    ...
    private CandidateResult result(DestinationCandidate c) {
        return new CandidateResult(
                ...
                c.note(),
                c.status().name(),
                store.preferences(c.id()).stream()
                        .map(DestinationCandidateService::preference)
                        .toList(),
                c.version(),
                c.createdBy(),
                c.createdAt(),
    ...
    private CandidateResult result(
            DestinationCandidate c,
            List<DestinationCandidateStorePort.PreferenceRecord> preferences) {
        return new CandidateResult(
                c.id(),
                c.tripId().value(),
                c.name(),
                ...
                c.note(),
                c.status().name(),
                preferences.stream().map(DestinationCandidateService::preference).toList(),
                c.version(),
                c.createdBy(),
                c.createdAt(),
                c.updatedAt());
