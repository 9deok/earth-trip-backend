...
final class StructureDiagnosticCalculator {
    ...
    static List<TripStructureUseCase.DiagnosticResult> calculate(
        ...
        return raw.stream()
            .map(diagnostic -> result(tripId, diagnostic, resolutions))
            .toList();
    }
    ...
    private static void addSegmentDiagnostics(
        ...
        if (!segment.type().equals("STAY")
            ...
            diagnostics.add(new RawDiagnostic(
                "INCOMPLETE_TRANSFER_TIME", "WARNING",
    ...
    private static void addNightDiagnostics(
        ...
        for (LocalDate night = tripStart; night.isBefore(tripEnd); night = night.plusDays(1)) {
            ...
            List<TripStructureView.Segment> stays = segments.stream()
                ...
                .toList();
            boolean overnightTransfer = segments.stream()
                .filter(segment -> segment.type().equals("OVERNIGHT_TRANSFER"))
                .anyMatch(segment -> !currentNight.isBefore(segment.startDate())
                    && currentNight.isBefore(segment.endDate()));
            if (stays.isEmpty() && !overnightTransfer) {
                diagnostics.add(new RawDiagnostic(
                    "EMPTY_NIGHT", "ERROR",
                    "숙소나 의도한 야간 이동이 없는 밤이 있습니다.", night, List.of()
                ));
            }
            if (stays.size() > 1) {
                diagnostics.add(new RawDiagnostic(
                    "OVERLAPPING_ACCOMMODATION", "ERROR",
                    "같은 밤에 숙소가 중복되어 있습니다.", night,
    ...
    private static TripStructureUseCase.DiagnosticResult result(
        ...
        return new TripStructureUseCase.DiagnosticResult(
            id, raw.code(), raw.severity(), raw.message(), raw.localDate(), raw.segmentIds(),
            resolution != null,
    ...
    private static UUID diagnosticId(UUID tripId, RawDiagnostic diagnostic) {
        String segmentKey = diagnostic.segmentIds().stream()
            .map(UUID::toString)
            .sorted()
            .collect(java.util.stream.Collectors.joining(","));
        String source = tripId + "|" + diagnostic.code() + "|"
            + diagnostic.localDate() + "|" + segmentKey;
