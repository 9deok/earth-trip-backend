...
@Entity @Table(name = "destination_candidates")
class DestinationCandidateJpaEntity {
    ...
    DestinationCandidateJpaEntity(DestinationCandidate c) { id = c.id().toString(); apply(c); }
    void apply(DestinationCandidate c) {
        tripId = c.tripId().toString(); name = c.name(); countryCode = c.countryCode();
        placeId = c.placeId(); latitude = c.latitude(); longitude = c.longitude(); note = c.note();
        status = c.status().name(); createdBy = c.createdBy().toString();
        createdAt = c.createdAt(); updatedAt = c.updatedAt();
    }
    DestinationCandidate toDomain() {
        return DestinationCandidate.restore(
            UUID.fromString(id), TripId.from(tripId), name, countryCode, placeId, latitude, longitude,
            note, DestinationCandidate.Status.valueOf(status), UUID.fromString(createdBy),
