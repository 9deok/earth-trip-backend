...
public interface DateCandidateUseCase {
    ...
    AvailabilityResult putAvailability(UUID tripId, UUID id, UUID actor, String availability, String note);

    record CandidateCommand(
        UUID requestId, LocalDate startDate, LocalDate endDate, String note, String status, long baseVersion
    ) { }
    record AvailabilityResult(UUID userId, String availability, String note, Instant updatedAt) { }
    record CandidateResult(
        UUID candidateId, UUID tripId, LocalDate startDate, LocalDate endDate, String note, String status,
        List<AvailabilityResult> availability, long version, UUID createdBy, Instant createdAt, Instant updatedAt
    ) { }
}
