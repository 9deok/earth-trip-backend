...
class DateCandidateJpaEntity {
    ...
    DateCandidateJpaEntity(DateCandidate c) {
        id = c.id().toString();
        apply(c);
    }

    void apply(DateCandidate c) {
    ...
    DateCandidate toDomain() {
        return DateCandidate.restore(
                UUID.fromString(id),
                TripId.from(tripId),
                startDate,
                endDate,
                note,
                DateCandidate.Status.valueOf(status),
