...
class TripSegmentService implements TripSegmentUseCase {
    ...
    public List<SegmentResult> list(UUID tripId, UUID actorUserId) {
        ...
        return segmentStore.findAll(new TripId(tripId)).stream().map(TripSegmentService::result).toList();
    }

    @Override @Transactional(readOnly = true)
    public SegmentResult get(UUID tripId, UUID segmentId, UUID actorUserId) {
        tripAccess.requireViewer(tripId, actorUserId); return result(loadInTrip(tripId, segmentId));
    }
    ...
    public SegmentResult create(UUID tripId, UUID actorUserId, SegmentCommand command) {
        ...
        int sortOrder = command.sortOrder() == null
            ? segmentStore.findAll(new TripId(tripId)).stream().mapToInt(TripSegment::sortOrder).max().orElse(-1) + 1
            : command.sortOrder();
        Instant now = clock.instant();
        TripSegment segment = TripSegment.create(
            command.requestId(), new TripId(tripId), type(command.type()), command.cityName(),
            command.countryCode(), command.placeId(), command.latitude(), command.longitude(),
            command.startDate(), command.endDate(), command.accommodationName(),
            command.accommodationPlaceId(), command.checkInAt(), command.checkOutAt(),
    ...
    @Override
    public SegmentResult update(UUID tripId, UUID segmentId, UUID actorUserId, SegmentCommand command) {
        tripAccess.requireEditor(tripId, actorUserId);
        TripSegment segment = loadInTrip(tripId, segmentId); verifyVersion(segment, command.baseVersion());
        segment.update(
            type(command.type()), command.cityName(), command.countryCode(), command.placeId(),
            command.latitude(), command.longitude(), command.startDate(), command.endDate(),
            command.accommodationName(), command.accommodationPlaceId(), command.checkInAt(),
    ...
    public List<SegmentResult> reorder(UUID tripId, UUID actorUserId, List<OrderItem> order) {
        ...
        if (order.size() != current.size() || order.stream().map(OrderItem::segmentId).distinct().count() != current.size()) {
            throw EarthTripException.badRequest("INVALID_SEGMENT_ORDER", "모든 구간을 중복 없이 포함해야 합니다.");
        }
        Instant now = clock.instant();
        for (OrderItem item : order) {
            TripSegment segment = current.stream().filter(it -> it.id().equals(item.segmentId())).findFirst()
                .orElseThrow(() -> EarthTripException.badRequest("INVALID_SEGMENT_ORDER", "다른 여행의 구간이 포함됐습니다."));
    ...
    private TripSegment loadInTrip(UUID tripId, UUID segmentId) {
        return segmentStore.findById(segmentId).filter(it -> it.tripId().value().equals(tripId))
            .orElseThrow(() -> EarthTripException.notFound("SEGMENT_NOT_FOUND", "여행 구간을 찾을 수 없습니다."));
    }

    private static TripSegment.Type type(String raw) {
        if (raw == null) throw EarthTripException.badRequest("SEGMENT_TYPE_REQUIRED", "구간 유형이 필요합니다.");
        try { return TripSegment.Type.valueOf(raw.strip().toUpperCase(Locale.ROOT)); }
    ...
    private static void verifyVersion(TripSegment segment, long baseVersion) {
        if (segment.version() != baseVersion) throw new EarthTripException(
            "VERSION_CONFLICT", 409, "다른 구간 변경이 먼저 저장되었습니다.",
    ...
    private static SegmentResult result(TripSegment s) {
        return new SegmentResult(
            s.id(), s.tripId().value(), s.type().name(), s.cityName(), s.countryCode(), s.placeId(),
