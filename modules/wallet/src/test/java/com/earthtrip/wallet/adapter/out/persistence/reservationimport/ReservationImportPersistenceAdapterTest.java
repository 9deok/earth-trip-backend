...
class ReservationImportPersistenceAdapterTest {
    ...
    void 예약_가져오기_원문은_평문을_남기지_않고_round_trip한다() {
        ...
        ReservationImportCandidateJpaRepository candidates = mock(
            ReservationImportCandidateJpaRepository.class
        );
        ...
        ReservationImportPersistenceAdapter adapter = new ReservationImportPersistenceAdapter(
            jobs,
            candidates,
            new EncodedSensitiveData(),
            JSON
        );
        ReservationImportStorePort.JobRecord record = new ReservationImportStorePort.JobRecord(
            UUID.randomUUID(),
            UUID.randomUUID(),
            "DOCUMENT_OCR",
