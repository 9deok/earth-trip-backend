...
class ReservationChangePersistenceAdapter implements ReservationChangeStorePort {
    ...
    ReservationChangePersistenceAdapter(
        ReservationChangeSetJpaRepository repository,
        ObjectMapper json
    ) { this.repository = repository; this.json = json; }
    @Override public Optional<ChangeSetRecord> find(UUID changeSetId) {
        return repository.findById(changeSetId.toString()).map(this::record);
    }
    @Override public ChangeSetRecord save(ChangeSetRecord record) {
        return record(repository.save(new ReservationChangeSetJpaEntity(
            record, write(record.beforeSnapshot()), write(record.afterSnapshot())
        )));
    }
