...
class ReservationChangeSetJpaEntity {
    ...
    ReservationChangeSetJpaEntity(
        ReservationChangeStorePort.ChangeSetRecord record,
        String before,
        String after
    ) {
        id = record.id().toString(); tripId = record.tripId().toString();
        reservationId = record.reservationId().toString();
        requestedBy = record.requestedBy().toString(); proposalHash = record.proposalHash();
        beforeSnapshot = before; afterSnapshot = after; appliedAt = record.appliedAt();
    }
    ...
    ReservationChangeStorePort.ChangeSetRecord toRecord(
        ...
        return new ReservationChangeStorePort.ChangeSetRecord(
            UUID.fromString(id), UUID.fromString(tripId), UUID.fromString(reservationId),
            UUID.fromString(requestedBy), proposalHash, before, after, appliedAt
        );
    }
}
